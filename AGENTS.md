...
| `E:` | 大文件下载、低频工具、归档 | 安装包、模型素材包、参考图集归档 |
...
23. 禁止提交密钥、本机路径凭据、UE 生成目录和大型非 LFS 二进制。**口径澄清（2026-09-12）**：本条的「**本机路径凭据**」指**可用于获得访问权的路径/账号信息**（如密钥文件路径及其内容、共享目录凭据、带 token 的 URL）；**不含「工具链可执行文件的绝对路径」** —— 后者是**可复现性依赖**，**必须写明**（否则换会话/换人后连门禁都跑不起来，正是规则 10「不得自行假设」要防的情形）。**判断标准：该路径泄露后能否被用来取得访问权 ⇒ 能则禁，不能则留。** 既有惯例一致：规则 28 本身含 `D:\App\trae\ue-mcp\1.0.87\...`，`.trae/CHANGELOG.md` 亦记录多处本机路径（均为历史事实或可复现性依赖）。
...
## 副本与可追溯（2026-09-12 用户裁定）

> **用户原话**：「**我比较避讳本地多处备份，因为这样会造成很多文件后期都不不知道哪份是最新的了**。」

29. **唯一权威副本 = 项目工作区 + git 远端。** **禁止在本地另存"备份"。** 需要防丢 ⇒ **提交进版本库（含 LFS）**；**不得用"复制到别处"代替版本管理**。**多份副本的真实代价 = 制造"不知道哪份最新"的第二份。**
30. **不允许长期存在无人认领的副本、分支或归档。** 任何一处**要么归并进权威线，要么删除**；确属 git 无法承载的（如超过 LFS 上限的原始素材）⇒ **必须登记"这是唯一副本"及其位置**。**"留着以后再说"本身就是下一次"不知道哪份最新"的种子。**
31. **判定冗余的准则（2026-09-12 已实用于 20 项清理）**：**存在逐字节等价副本 ⇒ 冗余，可删；无副本且承载决策或事故记录 ⇒ 保留。**
32. **远端分支纪律**：**主线之外的每个分支必须有明确用途与维护者**；无用途的分支（含陈旧分叉）**删除**。**当前 `master` = 主线（权威，唯一验收层）。**
    **⇒ 事实澄清（2026-09-12 实测）**：`gh-pages` **不是**看板部署产物 —— 它是 **2026-08-12 旧 Pages 提交式部署的残留**（顶端 `242f0d5`，仅 1 个提交，其上 `integrity.json` 是一个月前的 `overall: fail` 旧读数）；**现行 `.github/workflows/deploy.yml` 走 Pages API（`deploy-pages@v4`），不向该分支写提交**。**按本条该分支属「无用途」，应删除**（不可逆操作，见规则 9）。
