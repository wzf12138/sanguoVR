...
23. 禁止提交密钥、本机路径凭据、UE 生成目录和大型非 LFS 二进制。**「本机路径凭据」指可用于获得访问权的路径/账号信息**（密钥文件路径及其内容、共享目录凭据、带 token 的 URL），**不含工具链可执行文件的绝对路径** —— 后者是**可复现性依赖，必须写明**（否则换会话后连门禁都跑不起来）。**判断标准：该路径泄露后能否被用来取得访问权。**
...
27. **跨会话信箱纪律（AgentHub）。** 跨会话消息一律走全局信箱 `D:\App\trae\AgentHub\`（协议 `AgentHub/PROTOCOL.md`，全局 Skill：agent-hub）。检查点：会话开始后、认领任务前、修改共享文件后（向 `channels/registry-changes` 发广播）、阻塞或需拍板时（`type: question` **一律投 `inboxes/manager/`**，然后 `hub_await` 值班等回复，来信自醒，不得空等用户传话）。信箱只是传输：认领、决策、验收等治理后果仍须落 `STATUS.json` 与登记册。
...
29. **唯一权威副本 = 项目工作区 + git 远端。** 禁止在本地另存「备份」；需要防丢则提交进版本库（含 LFS），不得用「复制到别处」代替版本管理。
30. **不允许长期存在无人认领的副本、分支或归档。** 任何一处要么归并进权威线，要么删除；git 无法承载的（如超过 LFS 上限的原始素材）必须登记「这是唯一副本」及其位置。
31. **判定冗余：存在逐字节等价副本 ⇒ 冗余，可删；无副本且承载决策或事故记录 ⇒ 保留。**
32. **远端分支纪律：主线之外的每个分支必须有明确用途与维护者；无用途的分支删除。** 当前 `master` = 主线。
33. **活资产不得置只读。** 凡**会被自动化写入**或**可能正被编辑器打开**的资产，**永远不得设 NTFS 只读位** —— 只读会导致保存失败并终止编辑器。**已知清单与工具见 `governance/operation-discipline.md`；依据见 `registers/01-decision-register.md`。**
