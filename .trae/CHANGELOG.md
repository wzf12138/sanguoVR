...
- **⚠ 格式跨度使字面串判据失效 —— 立为同族第 11 例（2026-09-12，本日第 3 次同类假阴性）** —— 第十六提交抽验中，执行侧回报「**内容存在，但字面串口径不成立**」：判据串「`**【用户裁定 A】**`」原样判 `False`，而去加粗的「`【用户裁定 A】`」判 `True` —— **因为加粗跨度自「【用户裁定 A】」一直延续到行末，串两侧的 `**` 不构成独立加粗跨度**。**本日三次假阴性**：①台账第 13 行（反引号）②「截至 `1b0e34f` 已核验」（反引号）③本次（加粗跨度）⇒ **原「双口径（原样 / 去反引号）」覆盖不到加粗跨度这一轴。**
