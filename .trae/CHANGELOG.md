...
- **立法同族第 25 / 26 / 27 例（第 33 批，全部由执行侧自查上报）**：
  - **第 25 例 · 凭证缺失**：`-f` 格式串里的裸花括号（`{64}`）被当成占位符索引 ⇒ **打印行 3 次全部未输出**，**而判定本身正常（3/3 合法、未误触发停手）**。**⇒ 判据链每一环都对，唯一错的是"把结果说给人看"那一环** —— **而凭证缺失与"未执行"在外部不可区分。**
  - **第 26 例 · 凭证说谎（本日最危险）**：克隆复验打印「**克隆内全量 SHA256 与基准相同 = `0/3`**」—— **链式索引抛 `CannotConvertStringToInt32`、异常被吞进 `if` 条件 ⇒ 计数器停在初值 `0`** ⇒ **印出一个形状完全合法的错数**：**`0/3` 在任何读者眼里都等于「克隆内容与基准不符」= 一次数据损坏事故，而真值是 3/3 全等**（补测新克隆确认）。**⇒ 与第 25 例的分野**：**第 25 例让第三方无从判断，第 26 例让第三方判断错。**
  - **第 27 例 · 量具标签撞名**：探针 ⑦-a 两边"都算对了却对不上" —— 执行侧数的是「**章内条文条数 = 4**」，我指的是「**第 1 条内命中次数 = 1**」，**两个不同的量共用一个名字**。**⇒ 不是内容缺陷，是命名缺陷。**
