...
- **A｜操作纪律正文拆分**：`governance/operation-discipline.md` **519 行 → 5 个文件**，**按主题分置，索引与正文分离**：
  - `operation-discipline.md` **61 行**（转为**索引** + 决策授权 + 安全默认）
  - `push-and-network.md` **156 行**（推送决策 / 暂存边界 / 网络恢复 / CI 取证）
  - `judgement-discipline.md` **222 行**（量具 / 三态 / 停手分级 / 取数口径）
  - `writing-discipline.md` **58 行**（编辑锚点 / 标题形态 / 渲染层书写）
...
- **拆分守恒性已实测（非声明）**：以 `git show HEAD:` 取拆分前原文比对**标题集合** ⇒ **丢失标题 = 0**；正文行（非标题、非空行）**340 → 368**，增量 = 5 个新文件头部 + 索引表 + 期间新增的推送节奏条。
