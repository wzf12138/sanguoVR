...
## 2026-09-13（指挥台会话：AGENTS.md 精简与自毁事故 / 只读复发 / VRE 示例来源确权 / v4 退役 / 门禁加固立项）

> **起因**：用户指示「**主要 agents 每次要注入，所以我想你精简一点**」「**你处理你加的部分**」⇒ 精简指挥台自己往 `AGENTS.md` 里加的内容；随后按用户逐条裁定处理示例工程、只读锁、关卡退役与门禁加固。

- **`AGENTS.md` 精简（用户要求：降低每会话注入成本）**：**13,630 B → 10,982 B**，规则数 **33 条不变**、章节 **11 节不变**、编号条目 **37 条不变**，实测纯 LF（`i/lf w/lf attr/text eol=lf`）、CRLF = 0 ⇒ **每会话约省 689 token**。改动仅限指挥台自己撰写部分：规则 23 / 27 去标注压缩、`## 副本与可追溯` 全节改为一句式规则并把依据外移。**用户第 1–28 条原章节未动。**
- **★ 事故与恢复（如实记录）**：精简过程中，指挥台用一个 Python 脚本按 `"\r\n"` 切分并替换「唯一匹配行」，而该文件当时是 **LF/CRLF 混合**，导致整段 LF 内容塌缩为**一个 10,405 B 的「行」**，`lines[i]` 赋值把**整份文件**替换掉 ⇒ **13,630 B → 1,529 B**，丢失规则 1–22、24–28 与全部标题。**⇒ 而门禁当时照报 `30 项 / 通过 29 / 警告 1 / 失败 0`、`EXIT=0`。**
  恢复方式：经 `.bat` + `cmd /c git show HEAD:AGENTS.md > …` 取回**原始 blob（11,995 B / LF / 119 行）**，再用整份写入工具重建文件并重新施加三处合法改动；恢复后逐项复核（33 条 / 11 节 / 37 条编号 / `w/lf` / 门禁回绿）。
  **⇒ 本次事故直接催生 `CR-20260913-001` 的 D1 检查项**，并已立为 `judgement-discipline.md` §3.22 机械路径第 4 条：**改写文件前必须先取该文件的换行约定，写入时必须照用；大量结构文本的改写一律用整份写入工具，不用拼串脚本。**
- **连带清理：152 个换行虚报文件** —— 事故波及使 `git status --porcelain` 由 **17 → 173**。逐个判定后确认为**纯换行差异**（`git diff --stat` 为空、字节数与 `BOM` 索引/工作区一致），**152 个经 `git checkout --` 还原、0 失败**，余 1 个（`Config/DefaultEngine.ini`）手工还原。**⇒ 本日实测口径：107 个 ` M` 中**仅 19 个有真实内容差异**，88 个为虚报。**
- **★ 活资产只读复发与再处置**：白名单内 4 个活资产（`L_Prototype_1v1_v5.umap`、`L_SkeletonTest.umap`、`BP_VRCharacter.uasset`、`BP_VRGameMode.uasset`）**再次**被置为只读，而当时编辑器 **pid 15704** 正持有它们 —— **即 13 次崩溃中 3 次的触发条件**。已解只读并逐项回读断言：`Content` 只读数 **195 → 137**，8 个关键活资产**全部 `False`**、**仍只读的活资产 = 0**。
  **⇒ 重设只读的作者：未取到数**（NTFS 不记录属性变更时间）。**两个 `Unlock-*.ps1` 已证伪**（其白名单守卫对双向生效，`-Restore` 跳过白名单），**不是它们做的**。
- **VRE 官方示例工程来源确权（`AGENTS.md` 必读第 1 项同步）**：本机**两份**实测关系 —— **`E ⊂ D`**（D 源树 932 文件 / E 源树 842 文件 / **E 独有 = 0**）；229 个同名文件大小不同，抽样 **4/4** 验证**归一换行后逐字符相同**（字节差 == CRLF 差，31 / 73 / 12 / 7 全等）；**D 独有的 90 个 = 88 个本项目 `UE_MCP_Bridge` + 2 个字体** ⇒ D 是 E 的**超集且被本项目污染**，**非等价副本**。
  **用户裁定**：「**今天问了一圈，都说本地没有这个示例项目**」⇒「**到时候 AI 都从 E 盘去取示例**」⇒ 已写入 **`.trae/README.md`「外部参考工程（示例来源）」**：权威来源 `E:\AWork\Temp\VRExpansionPlugin\VRExpPluginExample-5.6\VRExpPluginExample-5.6-Locked\`（842 文件 / UE **5.6** / LF / **实测 0 只读**）**；D 盘那份「暂时留，哪天不想用了再删」**。
  **⇒ 同批登记成族**：**「未取到数」被当成「不存在」** —— E 盘那份自 **2026-08-16 17:52** 即存在，全程未被删除。
- **`L_Prototype_1v1_v4.umap` 退役（规则 30/31）**：`session-20260908-002` 独立判定 v5 为 v4 完整超集、v4 独有设计内容为零 ⇒ **用户裁定「1.删」**。**删除前先实证三前提**：① `rebuild_v4.py`（18,018 B，sha16 `59D7A6FB45BC3A85`）**已提交且在 `origin/master`**；② HEAD 与 `origin/master` 均含 LFS 指针（blob `b15c6749…`，oid `120e7f15dc5e53dd404d4ff10a1b3976ef564885e1b5f50076b29e8c102e1854`，size 509,749），**本地 LFS 库存在该对象且字节一致**、`git lfs fsck` = OK；③ **无其他资产引用它**。⇒ **恢复命令 `git checkout HEAD -- Content/VRSanguo/Dev/L_Prototype_1v1_v4.umap`（可逆性已实证）。**
- **`Content/VRE/` 入库获批**：36 个 `.uasset` / **25.4 MB**（无非资产文件、无同名冲突、全部满足 LFS 规则）⇒ **用户裁定「2.入库」**，理由：本项目动画蓝图已引用它，不入库则他人克隆后引用为断。**同批一并入库** `Content/VRSanguo/VR/Animations/`、`HandPoseDriverComponent.{h,cpp}`、两份审计报告。
- **裁定回写：不向上游提交反馈** —— 用户裁定「**5.不提反馈，能做的我们自己做**」⇒ 桥工具缺口**一律自行修复**，`read_bone_track` 疑似返回 ref 回退值等三项**转入 TD-016 / TD-019 同批的 `1.0.87-patch` 自改造**。
- **★ 立 `CR-20260913-001 门禁加固批次二`（用户批准：「可以，立项吧」）** —— 本批**破封** `check-integrity.py`（`77,269 B / A280C20A…4792`），新增 2 项、修正 2 项判据的分级：**D1 受管正文完整性检查**（本次事故暴露：编号递增 ≠ 正文完好）、**D2 可再生目录存在性检查**（废止「以 ` D` 行数为零」这一在忽略目录上恒为真的判据）、**D3 认领期白名单由失败降为警告**（TD-020，与规则 16 的时间冲突）、**D4 换行虚报可见化**。**验证要求三层：本机 + 干净克隆 + CI，且必须含负向对照。**
- **本批推送**：第 45 批已派 `git-operator`（真实内容改动 15 项 + 删除 1 项 + 新增 6 项；口径④ 编辑器资产 4 项**只报告、严禁暂存**；88 个换行虚报一并暂存后自动归一）。

## 2026-09-12（第 43 批：治理结构整理 —— 操作纪律拆分 / 规则⇄门禁映射 / 台账缺口补记）

> **起因**：用户问「治理规则体系架构本身你评估吗」 ⇒ 评估发现**立法能力远强于执法能力**，并列出四个结构性问题；**用户裁定「AB 都可以做，C 我不建议…我觉得一次性到位最好」，并追加原则：「我不喜欢冗余，已废弃的规则该删的就直接删，不要打补丁说这个已废止，那个待确认啥的」。**

- **A｜操作纪律正文拆分**：`governance/operation-discipline.md` **519 行 → 索引 + 4 份主题正文（共 5 个文件）**，**按主题分置，索引与正文分离**：
  - `operation-discipline.md` **62 行**（转为**索引** + 决策授权 + 安全默认）
  - `push-and-network.md` **156 行**（推送决策 / 暂存边界 / 网络恢复 / CI 取证）
  - `judgement-discipline.md` **222 行**（量具 / 三态 / 停手分级 / 取数口径）
  - `writing-discipline.md` **60 行**（编辑锚点 / 标题形态 / 渲染层书写）
  - `exception-taxonomy.md` **44 行**（清单外异常口径 ③~⑦）
- **★★ 按用户「废弃直接删、不打补丁标注」原则，本轮删除 6 处已废内容（无一处保留"已废止"字样）**：
  1. **整段重复**：「锚点即内容」的**背景 + 四条纪律在原文 L284–L292 写了两遍**（与 L259–L282 内容重叠）⇒ **删后段**；
  2. **孤儿条文**：§3.14「取属性前先赋值」的立法句被后插的 §3.18 切断，孤悬于 L466–L468 ⇒ **并回 §3.14 正文**；
  3. **连续 7 个空行**（原文 L470–L476）⇒ 删；
  4. **空标题**：原文 `## 安全默认` 下**无任何正文**，而其真实内容（AFS 密钥节）**错挂在"决策授权"之下** ⇒ **结构归位**，AFS 密钥节移入"安全默认"；
  5. **过时数字**：量具声明块要素数在文内**一处写"五要素"、一处写"六要素"**，而实际已立为**七要素** ⇒ **删去过时数字，统一指向 §3.8**；
  6. **裸行无归属**：文末 L519「用户意图无法明确归类时…」**无标题归属**，且与 `skills/…/SKILL.md`「意图不明确时进入只读模式」**重复** ⇒ 删本文一份（SKILL 保留）。
- **拆分守恒性已实测（非声明）**：以 `git show HEAD:` 取拆分前原文比对**标题集合** ⇒ **丢失标题 = 0**；正文行（非标题、非空行）**340 → 369**，增量 = 5 个新文件头部 + 索引表 + 期间新增的推送节奏条。
- **指针同步（7 处，逐处 `Contains` 命中后才替换）**：`SessionCommands.md` 索引表 4 行、`SKILL.md` 4 处、`registers/11-tech-debt-register.md`（TD-018）、`.trae/index.md`、`.trae/README.md`、`integrity.yaml`。**复扫「`operation-discipline.md` §推送 / §清单外 / §判据与文书 / §校验」旧形态残留 = 0**（排除 `CHANGELOG.md` 历史记录，**历史不回溯改写**）。
- **B｜新建 `governance/rule-gate-matrix.md`（规则 ⇄ 门禁覆盖关系）**：`AGENTS.md` **32 条规则逐条**登记「对应门禁项 / 执行者」。**实测覆盖**：**机器 3 条（规则 11/16/26）· 半机 5 条（14/19/20/21/23）· 人守 24 条**；另有 **14 项门禁不对应任何规则**（结构事实与门禁自指）。
  **⇒ 该表的作用**：**规则新增时必须登记一行，否则该规则没有执行路径** —— 把「规范靠人守 / 断言靠机器守」的分界线**从口号变成可查数据**。
- **规则 32 事实错误修正**：原文称 **`gh-pages` = 看板部署（工作流自动维护）**，**与实测不符** —— 该分支顶端 `242f0d5` **仅 1 个提交、日期 2026-08-12**，其上 `integrity.json` 为一个月前的 **`overall: fail`** 旧读数；**现行 `.github/workflows/deploy.yml` 走 Pages API（`deploy-pages@v4`），不向该分支写提交** ⇒ **`gh-pages` 是旧提交式部署的残留**。**正文改为事实陈述 + 按本条应删除**（**删除属不可逆操作，待用户单独裁定，本批不动远端**）。
- **★★ 台账缺口补记（本轮发现的真账目缺陷）**：台账原为 **1–36 行 + 第 41 行**，**第 37/38/39/40 四行从未写入** —— 而那四笔提交的提交信息**都写着"台账第 N 行 CI 回填"**，说明当时回填动作**只跑了一半**。**本轮补记四行 + 追加第 42 行** ⇒ **台账现为 1–42 连续，缺口 = 无**。补记行的 CI 列**据实写「未取证」并注明原因（当时未落 run id；补取受 403 配额阻断）**，**不以推测值填充**。
- **执行侧独立复算（不抄我的结论）**：`HEAD~1` 版旧 od = **504 行**（已入库冻结态）/ 正文行 340 / 标题 32；**旧标题逐字符命中 31/32**（唯一未命中 = 复合标题 `## 判据与文书纪律`，**按主题拆成 `## 判据纪律`+`## 文书纪律` 两条**，属改名非丢失）；**旧正文行命中 332/340**，8 条未命中经二次复查定性 = **元信息重写 3 + 同主题改写 3 + 跨文件留存 1 + 改名 1 ⇒ 真内容消失 0 例**。
- **声明值偏差 2 处（已按执行侧实测修正）**：`writing-discipline.md` 声明 58 ⇒ 实测 **60**（+2 = 原报告的正文行数，未计入我后加的 `## 文书纪律` 二级标题与空行）；`operation-discipline.md` 声明 61 ⇒ 实测 **62**（+1 同理）。**声明值取自我修结构之前的中间态，属我侧量具未更新，非内容差异。**
- **⛔ 本批 CI 未取到数**：executor 列表读 1 次 + 补读 1 次，**两次全 403**，**连 run id 都取不到** ⇒ 台账第 43 行 CI 栏只能写「结论待取（403，配额）」，**本批无 run id 可录**。
- **C｜通读 `knowledge/`+`standards/`+`vr/`+`systems/` 全量 2,793 行（43 文件）后修出的 3 处真矛盾**（**均由人工阅读发现，`check-integrity.py` 不查语义矛盾**）：
  - **护甲字段冲突**：`systems/index.md` 的 `UVRArmorDefinition` 关键字段写作「部位、防护、**耐久、破损**」，与最高权威 `knowledge/GameMasterPlan.md` §生命与护甲「**护甲没有耐久，不设计破损、修理、衰减或隐藏损耗**」及 `Design/HealthAndArmor.md` **直接冲突**。**若执行侧照此建数据资产，将造出总纲明令禁止的字段** ⇒ 已删去「耐久、破损」并改指权威正文。
  - **平滑移动定位冲突（用户裁定 A）**：`vr/04` 写「**通过 `IMovementMode` 预留**，作为**可选**舒适度设置」，`standards/07` 写「平滑移动为**可选项**」；而权威侧 `GameMasterPlan.md` §移动与身体「一期**同时正式支持**平滑移动和战术瞬移」、§完成定义「平滑移动与战术瞬移**可用**」、`Design/MovementAndFullBodyIK.md`「都是一期**正式能力**…**但不能删除平滑移动能力**」、`registers/04` **REQ-011「不得以「预留接口」替代」**、DEC-004 —— **全仓 15 处表述中仅这 2 个文件是少数派**。**`vr/04` 的「预留」不只措辞冲突，而是直接违反已登记的 REQ-011** ⇒ 两处均改为「**一期正式能力，须可切换、可用；默认关闭；不作教程/战斗的强制条件**」并补四处权威引用。
  - **用户对「是否新增语义矛盾门禁」的裁定**：**不加**（判定「感觉不好检查」成立 —— 全自动语义矛盾检查需理解语义，成本与误报不划算）。**采用最便宜档**：立条文 + 登记可机械检查路径，**不动封版门禁**。
- **D｜新增 `judgement-discipline.md` §3.21「权威定性句只允许出现在权威文件」**：**已由权威正文定性的概念，其他文件不得重述该定性，只能链接**。三例成因同一 —— **「同一件事实写在两个地方」**，正是根级 `AGENTS.md` 规则 19 要防的情形；**该条此前无任何执行路径**。
  **⇒ 机械检查路径已登记未接入**：扫描非权威文件的**定性句式**（`X 为…`/`X 是…`/`包含…字段`）命中即报警，属规则 19 的可机械化子集。**暂不接入的理由（据实）：3 例全部由人工阅读发现，尚未验证该判据能否拦住第二次；验证之前不改门禁**（不改动 = 不破封，避免重算封版三元组与全链复跑）。**`rule-gate-matrix.md` 规则 19 行已同步标注该未覆盖子集。**
- **门禁**：本批新增 5 个文件、链接 **170 → 175** ⇒ `integrity.yaml` 的 `checked` 声明同步 **175**。**终态 `30 项 / 通过 29 / 警告 1 / 失败 0 / EXIT=0`；链接 175 / 断链 0。**
...
- **本窗口提交与 CI 台账（十三段，红灯未掩盖、历史未重写、判据未放宽）**：
  ...
  | 24 | `97a15e9` | 立项判据工具「核对执行器」+ 唯一 Skill 挂钩（用户批准）+ 规格第 13/14 条 + 台账补记 23 行（3 files，+254−1） | #57 success | #57 success |
  | 25 | `9e7924e` | 推送决策正文 + 副本与可追溯纪律（AGENTS.md 规则 29-32）+ 唯一 Skill 挂钩 + 禁用截断文本归属判定 + 台账补记 24 行（5 files，+38−1） | #58 success | #58 success |
  | — | 远端分支 | **删除废弃分支 `main`**（`e5c8a9ae910658d24ec0a7da6f2a0500f6a774a8` / 2026-08-09 / `Initial commit`）：删除前完整取证、非强推、删除后远端仅 `gh-pages` + `master`、`gh-pages` 未动 | 非代码变更 | — |
  | 26 | `fad327a` | 立法同族第 18 例（报告义务被控制流短路 ⇒ 静默）+ 第 14 例首次实战拦住假红 + 工具规格第 15 条与 TC-13 + 台账补记 25 行与删分支行（2 files，+15−0） | #59 success | #59 success |
  | 27 | `520a5a4` | 立项 M00-T007 判据工具「核对执行器」任务包 + 台账补记 26 行 + 两条新可归因证据（9 files，+239−0） | #60 success | #60 success |
  | 28 | `d49e9a8` | 规则 23「本机路径凭据」口径澄清 + `-f` 参数禁止内联表达式 + 决策侧判据失误第 6/7/8 次登记（3 files，+20−2） | #61 success | #61 success |
  | 29 | `0a32773` | 立法同族第 19 例（取数失败被伪造成数值）+ 工具规格第 16 条与 TC-14 + 决策侧第 9/10 次登记（2 files，+15−0） | 与第 30 行同一次推送 | 与第 30 行同一次推送 |
  | 30 | `8965b97` | 用户保存后的三个资产入库（环首刀直版新资产 + 骨架测试关卡 + VR 角色，含 LFS 对象）（3 files，+7−4） | `34681925072` success | `34681925065` success |
  | 31 | `b2c3dcf` | 判据工具 M00-T007 入库（`verify-claims.py` 1,848 行 + 执行报告 2,243 行）+ 立法第 17/18 条与 TC-14~16 + 台账第 29/30 行更正（13 files，+4289−12） | `34682980620` success | `34682980583` success |
  | 32 | `b76af4a` | 判据立法收口（第 19~21 条 + TC-17/18 + 台账第 31 行 + 双方失误登记）（3 files，+92−2） | `34683486186` success | `34683486155` success |
  | 33 | `61ed453` | 分级停手立法 + 第 20 条产证行/格式串加严 + 量具命名唯一化 + 台账第 32 行（3 files，+51−4） | `34683773111` success | `34683773133` success |
  | 34 | `449afb0` | 量具六要素与剔除规则 + 「不可判定」/停手二维拆分 + 编号字面统一 + 台账第 33 行（3 files，+27−5） | 治理校验 #66 run `34683972227` success；看板部署 #66 run `34683972212` success（直读，head_sha 匹配） | 同左 |
  | 35 | `800a246` | 产证行三兄弟框架 + 锚点纪律升级为可机械检查 + 警告项命名 + 台账第 34 行（4 files，+49−3） | 治理校验 #67 run `34684247688` success；看板部署 #67 run `34684247701` success（直读，head_sha 匹配） | 同左 |
  | 36 | `69876bf` | 量具构造自断言 + 剔除逐探针 + 假通过定级 + 台账第 34/35 行（3 files，+26−2） | 治理校验 #68 run `34684496957` success；看板部署 #68 run `34684496962` success（直读，head_sha 匹配） | 同左 |
  | 37 | `81cfa9e` | 结构整理 —— `SessionCommands.md` 正文整段迁出至新建 `operation-discipline.md` + 冻结信号立法 + 台账第 34~36 行 CI 回填（9 files，+435−357） | 未取证（本行于第 43 批补记；当时未落 run id，补取受 403 配额阻断） | 同左 |
  | 38 | `f8d5e32` | 量具声明块升为七要素 + 跨 git 边界比对纪律（`git hash-object` 对 `rev-parse`）+ 台账第 37 行 CI 回填（3 files，+32−2） | 未取证（本行于第 43 批补记；当时未落 run id，补取受 403 配额阻断） | 同左 |
  | 39 | `b5b64b1` | 标题计数禁令机械化 + 修复两处标题自带计数的自我违反 + 台账第 38 行 CI 回填（2 files，+29−3） | 未取证（本行于第 43 批补记；当时未落 run id，补取受 403 配额阻断） | 同左 |
  | 40 | `3a93b83` | 空值判定与剔除规则双向纪律（附三形态×四判据矩阵）+ 取属性绑定顺序 + 台账第 39 行 CI 回填（2 files，+59−1） | 未取证（本行于第 43 批补记；当时未落 run id，补取受 403 配额阻断） | 同左 |
  | 41 | `9381780` | 文书书写形态（渲染层）与报零量具正负对照纪律 + 台账第 40 行 CI 回填（2 files，+47−1） | 治理校验 `34686835211` / 看板部署 `34686835221`（#73）｜**结论待取（403，配额）** —— 决策侧 2026-09-12 17:52 自行复读：API 403、无认证 HTML 页 0 命中 ⇒ 亦未取到数 | 同左 |
  | 42 | `82372af` | 汇总行只读与引用编号书写形态纪律 + 台账第 41 行 CI 据实登记（2 files，+33−2） | 治理校验 `34686835211` / 看板部署 `34686835221`（#73 补读）｜**结论待取（403，配额）** | 同左 |
  | 43 | `10f96ae` | 治理结构整理 —— `operation-discipline.md` 519/504 行拆为四正文（推送取证 / 判据纪律 / 文书纪律 / 异常口径）+ 索引 + 新建规则⇄门禁映射表（机器 3 / 半机 5 / 人守 24）+ 规则 32 `gh-pages` 事实澄清 + **台账 37~40 行缺口补记**（15 files，+659−486） | 治理校验 / 看板部署（#75）｜**结论待取（403，配额；本批连 run id 均未取到）** | 同左 |
...
- **判据工具 M00-T007 交付并验证通过（第 31 行入库）**：`verify-claims.py` **v1.1.0 / 1,848 行 / 93,546 B**（连续形态 `B975FAFD250F0BF83E3CE1607BBAB6038ED88FDF7D766C6DFBC46BCF4A93A9F0`；分组形态 `B975FAFD 250F0BF8 3E3CE160 7BBAB603 8ED88FDF 7D766C6D FBC46BCF 4A93A9F0`）+ 执行报告 **2,243 行 / 156,205 B**（`F1C4C229A286C281`）。**TC-01~TC-16 逐条通过、`--self-test` 14/14、门禁四次 `EXIT=0`、23 份判据日志 stderr 全 0 字节。** **⚠ 已实现并经验证，但尚未挂钩**（规格 §8：决策侧不得验收自己的设计，须独立复核后方可挂钩 ① 唯一 Skill 与 ② `AGENTS.md`）。
- **判据工具新增条文（第 19 / 20 / 21 条）与 TC-17 / TC-18**：
  - **§4.3 第 19 条「禁止凭已知部分补全未测值」** —— 任何写入文书的读数必须来自一次真实取数；**严禁按已知前缀 / 片段凭字形补全未测过的位**。与「取数失败伪造成 0」**同族但更隐蔽：值本身"看起来是对的"（已测部分是真的），而未被测过的部分与证据脱钩**。
  - **§4.3 第 20 条「哈希 / 指纹 / 计数必须自带格式自断言」** —— ①逐字符复制自取数输出（**禁止手打、禁止补全、禁止沿用旧读数**）；②**可执行三断言**（长度：SHA256 = 64 / `sha16` = 16；字符集：`^[0-9A-F]{64}$` 大写；不得跨文书沿用，须注明取数时点）；③**文书内部的分组计数之和必须 = 总数**。**⇒ 书写用「双形态并写」**：分组形态（8 组 × 8 位）供人读与长度自校验、连续形态供机读与复制进命令 —— **只写分组无法复制执行，只写连续无法肉眼校长**。**⇒ 哈希比对三分法**：长度非法 ⇒ 判「候选串非法」（不得据此判文件变化）；差异 ≤ 2 位且同侧 ⇒ 判「对方笔误」（雪崩效应下内容真变不可能只差 1–2 位）；差异 ≥ 3 位 ⇒ 转人工重取。
  - **§4.3 第 21 条「每份报告 / 每次执行开头必须有量具声明块五要素」** —— **量具**（命令或函数语义） · **范围**（被测集合显式定义，含锚点） · **时点**（本步之前 / 之后） · **分母**（**本机实测**；取自他方时显式标注「来自提单声明，未实测」并同时报实测分母） · **解析**（正则 / 剥离规则；先剥 ANSI 再以终止锚定符抓数；未命中 ⇒ 报「未取到数」）。**五要素缺任一 ⇒ 该项读数标「不可判定」，不折通过、不折失败、不阻断其余项。**（**本条由执行侧提出并采纳**：**让任一第三方只凭报告即可复算出同一读数，不依赖执行者的临场判断**。）
  - **§4.3 第 18 条扩充**：`at`（测量时点）适用范围由「数字 / 结构类」改为 **`contains` / `count_regex` / `shape` / `sha256` / `exec` 五类全部**；**缺 `at` ⇒ 整份判据文件不合法（退出码 3、不执行任何判据）**；新增③**比较基准必须显式写出**、④**抓数正则必须带终止锚定符**。**⇒ 修订理由（执行侧提，决策侧采纳）**：**存在性断言同样改变结论** —— 原型 (a) 恰是存在性断言（「本地 LFS 对象」在 `git add` 之前测），**只卡数字类会漏掉原型本身**。**理由原样采纳。**
  - **§7 新增 TC-17**（凭前缀补全未测值 ⇒ 必须判「不可采信」）与 **TC-18**（分组计数之和 ≠ 总数 / 哈希长度非法 ⇒ 判「文书自不一致」与「格式自断言失败」）。**TC 总数 16 → 18。**
- **立法同族第 23 例（新模态：值不是测出来的，是"推"出来的）**：**执行侧自查** —— 报告 §一 曾写入一个**没测过的全量 SHA256**，**按 `[:16]` 前缀凭字形补全**，同轮改为实测值。**⇒ 与「取数失败伪造成 0」同族但更隐蔽：前缀是真的、值看起来对。** ⇒ **由执行侧自查发现并修正**，故第 19 条条文**记名其来源**。
- **执行侧本轮 6 个读数缺陷（时序，全部自查上报，均产出"看起来合理"的数）**：① **时点错** —— 本地 LFS 对象断言跑在 `git add` **之前**（clean filter 未运行 ⇒ 对象本就不存在）⇒ 原读数作废，add 后重测 3/3；② **解析被 ANSI 打断** —— 门禁项数正则被转义切断 ⇒ **未取到数却输出空值伪读数**；③ **数组参与格式化 / 比较** —— `@(git rev-list --count …)` ⇒ `System.Object[]`，改用 **`[int]` 显式取整**后 = 0；④ **正则惰性通配吃进 ANSI** —— `通过:.{0,24}?(\d+)` 命中 `[0m` 里的 `0` ⇒ 「通过 = 0 / 警告 = 0」（**TC-16 ④ 的来源**）；⑤ **`-f` 内联 `$(if…)`** ⇒ 整行未输出（**违反其自己参与立法的裁 3**）；⑥ **分母取自他方文本未实测** ⇒ 输出「A 5/6、B 8/7」**形状像两处缺口、实为 13/13 全绿**（**TC-18 / 第 21 条⑤ 的来源**）。**⇒ ① ② ③ 已在早前第 16/17 条立法中登记，④ ⑤ ⑥ 为本轮新增；⑦ ⑧ 为第 33 批新增（见下）。**
  **⑦ 判据正确、凭证缺失** —— `-f` **格式串里的裸花括号**（`^[0-9A-F]{64}$` 的 `{64}`）被 PowerShell 当成**占位符索引 64** ⇒ **该行 3 次全部未输出**；**判定本身正常（3/3 合法、未误触发停手），只有"把结果说给人看"那一环丢了**。**⇒ 由此得出第 20 条 ④：产证行自身也必须有格式自断言**（**凭证缺失与"未执行"在外部不可区分**）。
  **⑧ 产证行印出一个形状完全合法的错数（本日最危险的一个）** —— 「**克隆内全量 SHA256 与基准相同 = `0/3`**」这一行：链式索引 `$T[$p][5]` 抛 `CannotConvertStringToInt32`，**异常被吞进 `if` 条件** ⇒ 计数器停在初值 `0`、`⚠` 分支也未打印 ⇒ **输出 `0/3`**。**⇒ `0/3` 在任何读者眼里都等于「克隆内容与基准不符」——即一次数据损坏事故，而真值是 3/3 全等**（已补测新克隆：3 目标全量逐字符相同）。**⇒ 归族 = 第 20 条 ④ 的升级实例**：**第 ⑦ 个是"凭证缺失"（外部无法区分算对没算），第 ⑧ 个是"凭证说谎"（外部会判断错）**。**⇒ 条文升级**：**产证行必须区分「取值失败」与「取值为 0」**；**计数器只允许在比较表达式成功返回布尔值后递增**；**表达式抛错 ⇒ 打印「未取到数」，禁止让初值 `0` 参与展示**。**⇒ 执行侧自查漏洞一并登记**：**它默认「脚本没抛错 = 判据链完整」，而 `$ErrorActionPreference='Continue'` 会让这类错误静默通过** —— **这是元层漏洞："我没有检查我有没有检查"。**
- **立法同族第 24 例（假红 · 门禁过度匹配）：Markdown 链接检查器把行内代码当链接。** 本批写文书时一处 PowerShell 片段写作 **方括号紧跟圆括号的链接形态**（**注意：本条目自身也因举出该字面而连触两次同样的判红 ⇒ 全文改为文字描述，不举字面**） ⇒ **门禁「Markdown 本地链接无断链」判 `断链 1 个：CHANGELOG.md → …`**，并连带触发「`integrity.yaml` 链接计数一致」失败（`broken 声明 0 ≠ 实测 1`；`checked 声明 163 ≠ 作用域内实测 164`）⇒ **门禁 `30 / 27 / 1 / 2 / EXIT=1`**。**⇒ 该检查不跳过反引号行内代码**（目标 `…` 只是省略号）。**⇒ 处置**：**`check-integrity.py` 保持封版不动**（判据不放宽），**改写法规避** —— 正文中**不得形成「方括号紧跟圆括号」的链接形态**（**本条目初稿即在正文里写出了该形态、又触发一次同样的判红 ⇒ 已改为文字描述，不再举出字面**），代码片段写作 `` `[int]` 显式取整 `` 之类。**⇒ 修后 `30 / 29 / 1 / 0 / EXIT=0`。** **⇒ 归族：与第 22 例（解析被 ANSI 打断）相反方向 —— 前者漏判、后者过判；两者共同点是「量具不区分"语法结构"与"字面字符」，故都属"解析层面"的读数缺陷。**
- **立法同族第 25 例 · 凭证缺失（第 33 批，执行侧自查上报）**：`-f` 格式串里的裸花括号（`{64}`）被当成占位符索引 ⇒ **打印行 3 次全部未输出**，**而判定本身正常（3/3 合法、未误触发停手）**。**⇒ 判据链每一环都对，唯一错的是"把结果说给人看"那一环** —— **而凭证缺失与"未执行"在外部不可区分。**
  - **立法同族第 26 例 · 凭证说谎（本日最危险）**：克隆复验打印「**克隆内全量 SHA256 与基准相同 = `0/3`**」—— **链式索引抛 `CannotConvertStringToInt32`、异常被吞进 `if` 条件 ⇒ 计数器停在初值 `0`** ⇒ **印出一个形状完全合法的错数**：**`0/3` 在任何读者眼里都等于「克隆内容与基准不符」= 一次数据损坏事故，而真值是 3/3 全等**（补测新克隆确认）。**⇒ 与第 25 例的分野**：**第 25 例让第三方无从判断，第 26 例让第三方判断错。**
  - **立法同族第 27 例 · 量具标签撞名**：探针 ⑦-a 两边"都算对了却对不上" —— 执行侧数的是「**章内条文条数 = 4**」，我指的是「**第 1 条内命中次数 = 1**」，**两个不同的量共用一个名字**。**⇒ 不是内容缺陷，是命名缺陷。**
- **立法同族第 28 例 · 量具过宽 ⇒ 假未命中（第 34 批，执行侧自查上报）**：探针⑥ 要的是「标题内**不含计数**」，**执行侧上线时实现成「不含任何数字字符」** ⇒ 把日期 `2026-09-12`（8 位）与「第 N 条」交叉引用一起判进去 ⇒ **读数「数字字符 = 9 ⇒ 未命中」= 假未命中**。三口径复算后剔除日期与交叉引用 ⇒ **残留 = 0 ⇒ 命中**。**⇒ 与第 27 例的对仗（执行侧原话）**：「**第 27 例是同名不同量，我这次是同量不同宽**」。**⇒ 它暴露出第 21 条只做了一半**：**要求了"量具要声明"，却没要求"量具要声明剔除规则"。** **⇒ 条文追加款**：**量具定义必须给出「剔除规则」** —— 凡「含 / 不含某类字符」类探针，**必须显式写明剔除什么**；**未给剔除规则 ⇒ 该项读数标「不可判定」。** **⇒ 危害等级低于第 25/26 例**（**未印出错误结论，只印出一次假未命中**），**但它是"读数对不上"的第三种成因，与第 27 例互补成一对。**
- **立法同族第 30 例 · 容器被覆盖 ⇒ 假命中（第 36 批，执行侧自查上报；本日最重）**：
  - **现象**：**A 段探针在正式运行中全部失效** —— ①③⑤⑥ 整行未输出（凭证缺失）；**② 印出「0/0/0 ⇒ 命中」**（**假通过**）；④ 印出全 0 错数。
  - **根因（已定位，非猜测）**：**PowerShell 变量名大小写不敏感 ⇒ 容器 `$M`（哈希表）与循环内元素 `$m`（pscustomobject）是同一个变量** ⇒ **元素赋值把容器覆盖成最后一个文件对象** ⇒ 后续 `$M[...]` 变成对 pscustomobject 做字符串索引 ⇒ **四个量全部未取到数，而依赖它们的探针失效。** **⇒ 第 35 批用逐个调用（无容器），本批首次引入容器 ⇒ 缺陷随新代码诞生。**
  - **⇒ ★ 危害定级 = 本日最高，执行侧原话直接采用**：**「第 8 例印的是『假失败（0/3）』，人会去查；本批 ② 印的是『假通过（命中）』—— 假通过会让人跳过复核。」** **⇒ 通用结论**：**假失败 = 浪费一次排查；假通过 = 取消一次排查。前者错在成本，后者错在防线。**
  - **⇒ ★ 它为什么骗过了分母自断言（执行侧原话）**：**「分母自断言只校验『分母对不对』，不校验『量具是否存在』。」** —— **而「命中 6 + 未命中 1 = 7」在结构上完全自洽。** **⇒ 这说明第 21 条只做了两层，缺的正是第三层。**
  - **⇒ 与第 27 例的分野**：**第 27 例是"标签撞名"（两个不同的量共用一个名字）—— 人看得出来**；**第 12 例是"容器被同名元素静默覆盖" —— 静默摧毁量具，且计数器保持初值 `0` ⇒ 假命中与凭证缺失同时发生。**
  - **⇒ 事故范围界定（执行侧判断，我同意）**：**只污染探针读数（只读操作），未污染提交内容** —— **暂存三道断言、索引==blob 4/4、回读 4/4、克隆 4/4、门禁 ×3 全部独立成立。** **⇒ 读数失效 ≠ 内容失效。**
- **★ 第 41 批推送成功（`9381780`，2 files，+47−1）· 探针 7/8**：**指纹 2/2 逐字符全等**；门禁三处 `EXIT=0`（链接 170 / 断链 0）；克隆内 2/2 blob 一致；**对照台 6/6 通过**。**⇒ 唯一未命中是字面形态（见下）。**
- **🔴 CI #73 结论<未取到数>（不是绿、也不是红）—— 三方都取不到，据实登记**：
  - **执行侧**：**首读 17:49:28 两 run 均 `in_progress`；追加 1 次 17:50:29 遇 `HTTP 403`（配额）⇒ 两次均未取得结论**；**它明确拒绝把该栏填 success（「那会是假通过」），并固定了 run id 供后查** ⇒ **处理正确。**
  - **决策侧（我，17:52 自行复读，两条路各 1 次）**：**路径 A `api.github.com` ⇒ `403 Forbidden`**；**路径 B 无认证 Actions 列表页 HTML ⇒ `9381780` 命中 0 条** ⇒ **亦未取到数。**
  - **⇒ 结论：我的出口没有二级'（已认证 HTML）能力，只有执行侧有** ⇒ **台账第 41 行 CI 栏据实写「结论待取（403）」，不折通过、不折失败**（**三态判据原则**）。
  - **⇒ 处置**：**接受执行侧提议 —— 下一批带 1 次读取补 #73 结论**（记次数）。
- **★ 执行侧第 23 缺陷 · 汇总行改写了被汇总的变量（本批最重的一条）**：**它写 `@($ctrl1,$ctrl2,$ctrl3,$ctrl4,$ctrl5=$hp -eq 1,$ctrl6)` —— 数组字面量里的内联赋值被解析成<多变量赋值>**。
  - **决定性复现**：`@($c1,$c2,$c3,$c4,$c5=42,$c6)` 执行后 ⇒ **`ctrl1..ctrl6 = 42, True, (空), (空), (空), True`** ⇒ **第一个变量被改成 42、中间三个被清空** ⇒ **其后所有引用全部变成假值。**
  - **后果两层**：① 该行 `.Count` 在标量上报错 ⇒ **凭证缺失**（#16/#21 家族）；② **后续每一处对照标注都印成「未通过」= <假失败标注>**（对照本身其实全部通过）。
  - **⇒ 立法（§3.17）**：**「汇总者不得改写被汇总者」** —— **汇总行必须只读：不得在汇总表达式里写任何变量赋值。** **⇒ 与 #13（容器被覆盖 ⇒ 假命中）同族，但这次被覆盖的是<断言变量自己>。**
- **执行侧第 22 缺陷 · 格式串错位**：**`-f` 的闭合括号提前 ⇒ 先打印格式原文、再把值分行打印** ⇒ **凭证「形态错」，但读数可从后续行完整读出**（37 / 0 / 2 / 0 ⇒ **实体命中**）。**⇒ 修正：`-f` 必须在 `Write-Output` 的括号内。**
- **★ 乙级 · 决策侧第 32 次（判据字面形态错，同日第二次）**：**我写「阳性对照」，文件里实际是「正/负对照」「正负对照」；我写「§3.15 发布后」，文件里是「（§3.15）发布后」（编号被括号包住）** ⇒ **执行侧按字面照测、照实报 0，并<逐个变体排查>后并列实体，未替我抹平。**
  - **⇒ 这次形态差的本质**：**「编号被括号包住」—— 恰好又是 §3.15（渲染层）的现场实例，只是发生在「引用编号」这种最常见的写法上。**
  - **⇒ 教训（补充立法）**：**提单里的字面判据，凡引用编号或自造新词，必须<从文件里复制实际串>；不得凭习惯加括号或空格。** **⇒ 执行侧的自救措施我采纳：下次按「编号出现 + 上下文关键词」两判据并列测。**
- **★ 第 40 批推送成功（`3a93b83`，2 files，+59−1）· 探针 8/8 全命中（本日首次满盘）**：**指纹 2/2 逐字符全等**；门禁三处 `EXIT=0`（**链接 170 / 断链 0，三处一致**）；克隆内 2/2 blob 一致；**CI #72 双绿**（`34686594458` / `34686594401`）。**⇒ 第 39 批引入的 2 处假链接确认已修好（见下 ③ 的对照证明）。**
- **★★★ 执行侧新增自设纪律：报 0 的量具必须先做<正/负对照>（本日最有价值的一条，直接入法）**：
  > **「机器守断言，<对照守机器>。」**（执行侧原话）
  - **问题**：**「0」既可能是「真的没有」，也可能是「量具瞎了」—— 两者在输出上长得<一模一样>。**
  - **它本批的执行（我没有要求，是它自己加的）**：**针对我上批的 2 处假链接，它读到 0，于是把<我上批那两处形态原样重建>做正负对照**：
    - **阳性 ①**：裸写「方括号紧跟圆括号」的代码原形 ⇒ **真候选 = 1 ✅ 抓住**
    - **阳性 ②**：真断链（指向一个不存在的相对路径） ⇒ **真候选 = 1 ✅ 抓住**
    - **阴性 ①**：同一串<包在行内代码里> ⇒ **0 ✅ 正确剔除**
    - **阴性 ②**：存在的相对路径链接 ⇒ **0 ✅ 正确剔除**
    - **阴性 ③**：纯锚点 ⇒ **0 ✅ 正确剔除**
    - **⇒ 灵敏度：阳性 2/2 抓住｜阴性 3 正确剔除 / 0 误报** ⇒ **本批的 0 = 「已修好」，不是「抓不住」。**
    - **⚠ 连带事故（决策侧当场复发）**：**我在本条里照抄了那两个坏样本的<原形>，它们在 Markdown 里<真的变成了链接>** ⇒ **门禁当场判红：`172 个链接，断链 2 个`** ⇒ **已改为纯文字描述并复测 170 / 0**。**⇒ 这是「文书书写形态」法则（§3.15）发布后<十分钟内>的第一次自我违反 —— 同一类错误，一天内在我身上发生两次。**
  - **⇒ 立法（§3.16）**：**凡报 0 / 报空 / 报「未命中」的量具，必须附一份<正负对照>**（阳性须包含**已知坏样本**、阴性须包含**已知好样本**）；**未附对照 ⇒ 该项读数标「不可判定」。**
  - **⇒ 层次递进（本日结论链的第四层）**：**规范靠人守 → 断言靠机器守 → <对照守机器>。**
- **★ 执行侧「量纲如实标注」的一处正确做法**：**门禁的 `170` = <全仓口径>；它的探针 ② = <本批 2 文件口径>**（实测：CHANGELOG 全部链接形态 5 处、**均 http**；operation-discipline = 0 处）⇒ **它明确声明「两者不是同一分母，不得互相替代」。** **⇒ 这是 §3.9「分母必须界定」在实践中的正例。**
- **★ §3.12 的<可对照实证>（同日、同数据、只换一行写法）**：
  - **同一情形（非目标文件无改动）、同一数据**：**第 39 批用 `-eq ''` ⇒ 判<未命中（假失败）>；第 40 批用 `IsNullOrEmpty` ⇒ 判<命中（真通过）>。**
  - **⇒ 这不是「谁认为新法更好」，而是<同一个量具、同一份数据、换掉那一行写法，结论就翻回来了>。**
  - **⇒ 执行侧的判词（原样采用）**：**「§3.12 是我今天见过的最短路径立法：<一行改法，直接消掉一类假失败>。」**
- **★ 第 39 批推送成功（`b5b64b1`，2 files，+29−3）**：**指纹 2/2 逐字符全等**；门禁三处 `EXIT=0`；克隆内 2/2 blob 一致；**CI #71 双绿**（`34686352889` / `34686352885`）。**⇒ 探针按字面 6/8，实体 8/8；两处未命中都是执行侧自己的量具缺陷，且它已定位到机制。**
- **★★ 今日对称性第四个实例（执行侧原话，本日最重要的综合，直接采用）**：
  > **「判据的完整定义 = 「量什么」+「不量什么」；只写前半句的量具，一定会测出一个数，而那个数是不是你要的数，取决于你没写的那半句。」**
  - **四例**：① 跨行 `aria-label`（执行侧误报）↔ 我的判据未声明**文本形态**；② CRLF/LF 回读（执行侧 #17）↔ 我的条文未含**行尾**；③ 我的首版标题扫描器过宽（**假命中**）↔ §3.11 未含**剔除项**；④ **本批：我的扫描器过宽（假命中）与它的 ⑦ 探针过宽（假未命中）<同时出现>** —— **一侧放宽 ⇒ 多报；另一侧放宽 ⇒ 漏报；同一根因。**
- **★ 执行侧第 19 缺陷 · 假未命中：空值判定形态（附决定性三×四矩阵）**：**它把 ⑥ 的判据写成 `$msState -eq ''`（原生命令无输出 = "无改动"）** ⇒ **四个子条件全对（88 行 / 6 / 12、索引 1、正文标题 0、工作区无改动），却因空值判定形态而判「未命中」。**
  - **决定性复测矩阵（三形态 × 四判据）**：
  ...
  - **⇒ 立法：空值判定<禁写 `-eq ''`>，一律用 `[string]::IsNullOrEmpty($x)`**（**三形态下唯一全为 True 的判据**）。
  - **⇒ 同时否掉一个常见写法**：**`@($x).Count -eq 0` <不通用>** —— **显式 `$null` 会被 `@()` 包成 1 个元素**（矩阵第 2 行）。
  - **⇒ 同族于 #17：同一条铁律「形态变了，量具就得变」—— #17 是<行尾>，本条是<空值>。**
- **★ 执行侧第 20 缺陷 · 剔除规则缺失（与我的首版扫描器同族，方向相反）**：**它按字面数「3.8」得 4（我声明 1）**：`L341` = 真标题；`L185`/`L365`/`L368` = **3 处交叉引用**。**且它那条「标题内不含数字」的子判据把<章节编号>与<日期>也算成计数 ⇒ 过宽。**
  - **⇒ 它按剔除规则<独立复算>（用自己的模式，不抄我的扫描器）**：**22 个标题按「N要素 / （N 项）/ 第 N 次」模式命中 = 0 ⇒ 我的「真违规 = 0」成立** ✅；且 `3.8`=L341｜`3.9`=L350｜`3.10`=L355｜`3.11`=L361 各 1 ✅。
  - **⇒ 立法：§3.11 的剔除项<同样适用于执行侧的字面探针>**（**判据的剔除规则是双方共同义务，不是文书作者的专属义务**）。
- **★ 执行侧第 21 缺陷 · 凭证缺失（第 8/16 例家族）：「方括号紧跟圆括号」的解析陷阱**：**PowerShell 把成员访问<先绑定到 `$x`>** ⇒ **写成「方括号紧跟圆括号」时，它会先把 `$x.Length` 求值再转 string（绑定顺序反转）** ⇒ **`$x` 为 `$null` 时整行抛错** ⇒ **该行凭证在输出里<静默消失>**。**⇒ 修正：先赋值再取属性；凡「取属性」类表达式一律显式加括号，并在打印前断言该行确有产出。**
  - **⇒ 连带教训（本例写成 CHANGELOG 时立刻复发一次）**：**在 Markdown 里照抄这种「方括号紧跟圆括号」的代码原形，会被<链接解析器>读成链接** ⇒ **本批因此让门禁断链 2 处（`CHANGELOG.md` 与 `operation-discipline.md` 各 1）** ⇒ **修法：描述绑定顺序时用文字拆开写，不要照抄代码原形。** **⇒ 这是「形态变了，量具就得变」在<文书书写层>的又一次现身。**
- **★ 第 38 批推送成功（`f8d5e32`，3 files，+32−2）+ 两条新法**：**指纹 3/3 逐字符全等**；门禁三处 `EXIT=0`；克隆内 blob 3/3；**CI #70 双绿**（`34686130342` / `34686130322`）。**⇒ 探针 7/8，唯一未命中是判据字面形态（见下）。**
- **★ 新法首跑即生效的两条实证（重要）**：
  1. **第 17 缺陷修正法（跨 git 边界比对）当场救回一次假失败**：**CR 与 operation-discipline 的「原始字节 sha256 相等」= `False`，而 `blob(过滤器口径) == HEAD:path` = `True` 且「行数组全等」= `True`** ⇒ **按旧法会被判<假失败>，按新法判<真实一致>。** **⇒ 这是该修正法的第二个现场证据（CRLF 工作区 vs LF blob，内容零差异）。**
  2. **CI 取数新法首跑**：**首读 17:32:50 两 run 均 `in_progress` ⇒ 判「未取到数」（不折通过、不折失败）；追加 1 次于 17:33:51 取到结论 success** ⇒ **实读 2 次，已按纪律记录。**
- **★ 立法自证实例 · 我在自己立法的同一个文件里违反了它（执行侧发现，本批之前既有）**：
  - **同文件内两处并存**：**`## 判据与文书纪律（…；<计数不写入标题>，见本章 §四 第 4 条）`**（**我亲立的规矩**） vs **`### 一、量具声明块（<五要素>…）`**（**标题里带计数，且已过期两级：五 → 六 → 七**），而 **`### 3.8 量具声明块<第七要素>…`** 正是现在的实际形态。
  - **⇒ 规矩要防的事，发生在规矩的同一个文件里。** **⇒ 与今日两条结论完全同向：「<规范靠人守，断言靠机器守>」「<法律写在文档里，不写在执行路径上>」。** **⇒ 这是该结论的第三个实例，也是最锋利的一个：立法的作者本人在立法文件内违反立法。**
  - **处置**：**`### 一、量具声明块（要素构成见 3.8；…）`**（去计数）+ **`### 3.8 量具声明块·「文本形态与归一化」要素`**（去计数）；**全库复扫，真违规仅此 2 处**（CHANGELOG 历史日期标题与 CR §4.3「由 17 例反推」不在本法则作用域内）。
  - **⇒ 由此得新法（可机械检查）**：**「标题不得含计数」必须由脚本按 `^#{1,6} .*(数字或数词)+(要素|条|项|例|次|组|个)` 模式扫描**；**光写成条文，连作者自己都会违反。**
- **乙级 · 决策侧第 31 次（判据字面形态错）**：**我在提单里写 `§3.8` / `§3.9` / `§3.10`，而文件实际的标题形态是 `### 3.8 …` / `### 3.9 …` / `### 3.10 …`（无 `§`）** ⇒ **执行侧按我的字面照测、照实报 `0`，未替我把 `§` 抹掉**（**它的原话**：**「否则就是替你改判据」**）⇒ **处理正确，是我的判据字面写错。** **⇒ 教训：引用章节时，判据字面必须<复制文件中的实际标题串>，不得凭记忆或凭习惯加标记符号。**
- **★ 第 37 批推送成功（`81cfa9e`，9 files，+435−357）**：**结构整理完成并入库** —— **九项冻结指纹 9/9 逐字符全等（我的基准零偏差）**；门禁三处（提交前 / 提交后 / 克隆内）**全部 `30/29/1/0 / EXIT=0`**；克隆内 9/9 blob 逐一相等；**CI #69 两工作流 success**。**⇒ 8 个实体探针全命中；「正文全部迁入 operation-discipline、SessionCommands 只留指令 + 索引」已被机械验证成立。**
- **★ 执行侧第 17 缺陷 · 回读量具对行尾不敏感 ⇒ 假失败（第 37 批自查）**：**`$w.Sha`（工作区原始字节，CRLF）vs `$b.Sha`（`git show HEAD:path` 输出，git 已归一为 LF）⇒ CRLF 与 LF 的 SHA256 天然不同 ⇒ 3 个 CRLF 文件被判"回读不符"**；**脚本在推送前抛出（方向安全：假失败只浪费一次排查，若反向放行真差异则不可逆）。** **正确判定**：**逐行内容全等 9/9（`ReadAllLines` 与行尾无关）+ `git hash-object --path` == `HEAD:path` blob 9/9 ⇒ 内容零差异，仅行尾归一化。**
  - **⇒ 立法 · 跨 git 边界比对（重要）**：**一律用 `git hash-object --path=<p>`（走过滤器）对 `git rev-parse <rev>:<p>`**，**并以「行数组逐行相等」为第二判据**；**不得直接比原始字节 SHA256。**
  - **⇒ 立法 · 量具声明块第 7 要素「文本形态与归一化」必须明确列出<行尾（CRLF/LF）与编码>**。**⇒ 本批是它的第二个真实实例：上一次是"正则不容换行"（第 14 缺陷、本例同族），这次是"哈希不认行尾"—— <形态变了，量具就得变>。**
- **执行侧第 18 缺陷 · 读数错位（§4.3 一度印成 32）**：**取的是 §4.3→§4.4 的<行跨度>，不是条目数**；**原因 = 改写探针时把取值表达式从"行数统计"换成"行号差"而未复核语义**（同族于第 11 例「产证行标签错位」）。**修正后 §4.3 = 21 ✅（口径 = `| 数字 |` 行计数）。**
- **★ 立法 · 台账行量具必须声明「占位行是否计入」**（与「行口径」并列，属追加款二的具体化）：**本批我写"末行含（本批）"，而执行侧的行量具 = 「N + `sha` 形态」⇒ 占位行无 SHA、不进其行集 ⇒ 字面判据不成立（实体是对的，是判据未声明）。**
- **★ 立法 · CI 取数纪律补充**：**「首次读到 `in_progress` ≠ 取到数；为取得<结论>允许追加读取，但必须记录实际调用次数。」** **⇒ 本批实际 2 次（首读时两 run 仍 `in_progress`、无结论；61 秒后追加 1 次成功），执行侧已如实记录。**
- **★ 立法 · 「残留」的分母 = 我自己创建的条目，不是目录里的条目**（执行侧第 37 批自报）：**它一度把 `tmp` 目录的 7 个既有条目（他会话 / 前日产物）报成自己的临时残留** ⇒ **分母过宽，属「分母未界定」同族。**
- **乙级 · 决策侧第 30 次（我自己的提单形态错误）**：**第 9 项 `sha256` 栏我写成「6153301507BA68CC 其余见前：615330…」——串内含「其余见前：」字样，不可解析** ⇒ **执行侧取可解析的 64 位串并按 sha16 交叉核对后继续（处理正确）。** **⇒ 教训：sha256 必须<连续 64 位原样>给出；不得在同一字段里混入说明文字。**
- **★ 结构整理：`SessionCommands.md` 正文迁出（2026-09-12，用户直接指示）**：**用户原话**：**「你为啥在 SessionCommands.md 里面写了这么多推送相关的内容，感觉你都写了一篇论文了」**、**「简短的部分放简短的说明和索引，复杂的地方可以另外开一个文件」**。
  - **实测**：**该文件当天 89 行 → 420 行（+275 / 删 0）**，其中 **350 行是执行侧实现规格**（哈希算法、`-f` 花括号陷阱、PowerShell 大小写、容器断言……）。**⇒ 我把<实现方案>写进了<给用户看的指令文件>。**
  - **⇒ 它同时违反两条既有规则**：① 项目唯一 Skill 自身声明「**不复制治理、产品、标准或系统规则正文**」；② `AGENTS.md` 规则 19「**一个主题只有一个权威正文，其他位置只链接**」。**⇒ 我还亲手在 SKILL.md 里写了一句「`SessionCommands.md` §推送 是该小节唯一权威正文」—— 把一个指令清单升格成了权威正文。**
  - **处置**：正文整段迁出至新建 **`.trae/governance/operation-discipline.md`**（363 行 / 57,072 B）；`SessionCommands.md` **压回 88 行**（原 89 行规模，**前 68 行指令区逐字未动，有断言**）+ 新增「纪律索引」表；**指针级联更新 5 处**（SKILL.md 四份操作模型与 §校验、`.trae/index.md`、`.trae/README.md`、TD-018）。
  - **⇒ 门禁当场抓到这次重构**：**第 14 项 `integrity.yaml 链接计数一致` 实测判红**（声明 163 ≠ 实测 169 ⇒ 随后 170），**更新声明后即转绿** —— **⇒ 这是门禁自身的"计数一致"断言第一次被观察到真实命中；与第 31 例「靠命名规范防不住，必须靠运行时断言」同向，登记为「断言有效性的第二个实例」（标注：强推论，依据 = 时间线 + numstat + 两次数值，非直读其内部字段）。**
- **★ 冻结延迟立法（第 37 批事故直接产出）**：**第 38 批提单下发后，我仍继续写入目标文件（`SessionCommands.md` / `integrity.yaml` / `SKILL.md` / `README.md` / `index.md` / `11-tech-debt.md`），致执行侧在同一窗口内读到两个相反状态（porcelain 5 → 7 → 11、门禁 `EXIT=1` → `EXIT=0`）。** **⇒ 它没有判成"决策侧写错了"，而判成「<窗口内没有冻结态>」—— 原话**：**「基线的前提是『取数时对象不动』。」** **⇒ ⇒ 这条判断是对的：移动靶不是坏基线，是<根本没有基线>。**
  - **立法 · 冻结信号（甲级新增判据）**：**提单必须显式给出「<已停手>」信号**（或明示"这些文件我不再改"）**后方可开始取数**；**提单下发 ⇒ 决策侧停手**；**窗口内若决策侧仍需写入 ⇒ 不得开窗口，须先作废该批、写完再重出基准。**
  - **立法 · 量具必须声明「物理行 vs 逻辑行」（追加款二的具体化）**：**凡"行尾是否闭合 / 行数"类探针，必须声明按物理行还是逻辑行量**。**⇒ 来源**：执行侧报「第 20 条行未以 `|` 闭合」，而实测**该条为多行表格逻辑行（L109..L117），末物理行 L117 以 ` |` 正常闭合** ⇒ **判「假警报（量具未声明行口径）」**；**第 21 条同为多行逻辑行（L118..L119，L119 为空行）** ⇒ **两行均正确闭合。**
- **执行侧第 16 缺陷 · 构造断言自身被语法特性吃掉（第 37 批自查）**：**`$TAGS=@(...)` 后写 `($TAGS | Where-Object {...}).Count` —— 零命中时管道返回 `null`** ⇒ **`Set-StrictMode -Latest` 抛「The property 'Count' cannot be found on this object」** ⇒ **「标签清单构造自断言」整行未能印出（凭证缺失）**，**而 `$listOk` 变量本身为真、不会 throw** ⇒ **若指纹当时相符，探针④会在缺构造断言的情况下继续运行 ⇒ 立法 1 被绕过。**
  - **⇒ 立法：凡管道结果取 `.Count`，必须用 `@()` 包裹**（`@($x).Count`）。**⇒ 同族第 8/12/13 例；本条是「构造断言自身被语法特性吃掉」—— 防伪造的机制自己被语言特性绕过。**
- **立法同族第 31 例 · 同名撞名第二次实例（第 37 批，执行侧自查上报）**：**执行侧在本批已按立法三把容器 `$M` 改名 `$SRC`、元素改名 `$one` —— 却把另一个容器写成 `$EXP`、元素写成 `$exp`**（**PowerShell 大小写不敏感 ⇒ 同一变量**）⇒ **克隆校验第一轮把容器覆盖成字符串** ⇒ 第二轮 `$EXP[$p]` 变成「用字符串索引字符串」⇒ 抛异常 ⇒ **`$exp` 保留上一轮的值 ⇒ 比较恒为 False** ⇒ **印出「克隆内一致 = 1（不符 2）」，真值 3/3 —— 这是<假失败>（第 8 例同族，方向安全）。**
  **⇒ ★★ 对称性结论（执行侧原话，本日最重的一条，直接采用）**：**「承诺式纪律 4 次失效（决策侧）+ 1 次失效（执行侧）= 同一个根因：<法律写在文档里，不写在执行路径上>。」**
  **⇒ ⇒ 它的意义**：**同一天、同一份文书、双方各自立法、各自违反** —— **说明问题不在执行者的自觉，而在<立法的位置>。** **⇒ 与第 27 次的结论合并**：**「把纪律放在人的前面，不如放在编辑的后面」** —— **两条合起来 = 本次全部失误的统一根因。**
  **⇒ 机械修正（非承诺）**：容器改名 `$REFSHA` / 元素 `$curHash`（词法不同）+ **容器完整性断言（取数前后各断言类型与条目数）** ⇒ **重测克隆内 3/3 一致**，容器断言 True。
- **★ 本日通用结论（由第 31 例的立法理由得出）**：**规范靠人守，断言靠机器守。** —— **「词法不同是<命名规范>，会被违反；容器类型与条目数是<运行时可断言的事实>，不会。」**（执行侧原话）
- **★ 三条立法（第 36 批事故直接产出，第 37 批入库）**：
  - **立法一 · 量具构造自断言（Instrument-Construction Assertion）**：**凡计数探针，取数前必须输出「量具构造自断言」一行**（含量具名 / 必需字段齐备 / 行数 ≥ 1）；**断言不通过 ⇒ 判「不可判定」并 fail-closed 中止**，**禁止以初值 `0` 参与展示**；**未输出该行者，该项读数一律判「不可判定」。** **⇒ 与分母自断言的分工**：**分母自断言证明"分母对不对"；量具构造自断言证明"量具在不在"—— 两者缺一不可。**
  - **立法二 · 剔除规则必须逐探针声明，不得全局套用**：**第 34 批立的全局剔除规则③（计数类探针剔除代码围栏行）与"围栏内断言块"类探针直接冲突** —— **该探针的被测对象正是围栏内的内容，剔掉必然假未命中**（第 36 批实测发生）。**⇒ 剔除规则逐探针声明；对"围栏内内容"类探针，必须显式写明"本探针不剔除围栏行"。**
  - **立法三 · 命名唯一化的强度升级**：**必须保证"词法上不同"，禁止依赖大小写区分**（容器与元素必须用不同词，如 `$SRC` / `$one`）。**⇒ 理由：`$M` 与 `$m` 在词法上不同、在 PowerShell 里是同一个 —— 靠大小写区分的唯一化等于没有唯一化。**
- **立法同族第 29 例 · 凭证错位 + 「产证行三兄弟」框架（第 35 批，执行侧自查上报）**：
  - **第 10 个 · 分母自断言的时序错位**：**汇总行打印在 ⑦（门禁探针）判定之前** ⇒ 打印「命中 6 + 未命中 1 = 7」，**而 ⑦ 随后判「命中」** ⇒ **真值 7/7 与产证行 6/7 不一致，读者会以为有一项未命中。** **⇒ 根因：汇总行没有放在"被汇总集合的最后一项之后"。**
  - **第 11 个 · 产证行标签错位**：探针⑤ 打印「第 21 条块 L118→**L-1**」—— **`L-1` 是哨兵值（-1 表示"不存在第 22 条行"）被原样打印** ⇒ **印出一个不存在的行号。**
  - **⇒ ★「三兄弟」框架（执行侧原话，直接采用）**：**第 7 例「凭证缺失」/ 第 8 例「凭证说谎」/ 第 10·11 例「凭证错位」—— 判据与取数都对，只有"给人看的那一行"错了。**
  - **⇒ 框架价值**：**它把"产证行"提升为一个独立的失效面** —— **「计算 / 判定 / 呈现」三段，前两段对不保证第三段对。** **⇒ 条文补写**：**④-a 分母自断言必须紧跟在被汇总集合的最后一项之后；判定分散多段时必须在全部段结束后再汇总（禁止跨段提前汇总）**；**④-b 凡可能的哨兵值（-1 / 0 / null）必须显式转成文字，禁止原样打印。**
- **★ 门禁那 1 项警告首次被命名（第 35 批，此前 35 批一直未命名）**：**`! 五件套结构（根↔目录↔taskId↔ALLOWLIST）`** —— **这就是每批「警告 1」的来源（不阻断，`EXIT` 仍为 0）。**
  **⇒ 同时纠正决策侧一条错账（第 25 次）**：**我的记录写的是「警告 = M00-T005 / M00-T006 的 6 条历史路径，BY DESIGN」—— 错的。** **⇒ 我此前 35 批一直在用一条未经核对的旧印象记账，正是"基准数字零转录"要防的事：我记的是"我以为的警告"，不是"实测输出里的警告"。**
  **⇒ 固定探针（下一批起）**：**A 段必须逐字符回报「警告」项原文** —— **不允许再用印象代替。**
  **⇒ 与 TD-020 的关系**：**同属「五件套结构」检查面，是同一个检查项的正反两面** —— **未认领时的历史缺口（警告）vs 已认领后的待产出文件（曾判失败）**。
- **CI 直读配额纪律（第 35 批实测得出）**：**未认证 GitHub API 限 60 次/小时**；**本小时三批（33/34/35）各调 2–3 次 ⇒ 配额耗尽 ⇒ 直读 3 次全 403。** **⇒ 纪律**：**CI API 调用按小时配额预算，每批直读 ≤ 2 次（两条工作流各 1 次）**；**超限即报「配额受限」并改用二级证据，禁止重试刷屏**（**重试只会让配额恢复更慢**）。 **⇒ 二层证据分级（第 35 批实测）**：**一级 = SHA 直读 API**；**二级 = 分支徽章 passing + 时序唯一性证明**（`origin/master == HEAD` 且推送后无更新提交 ⇒ 分支最新一次运行必然属于本批）—— **二级必须显式标注「强推论，非直读」。**
- **关键判据两套独立算法交叉复核首次自然发生（第 35 批）**：**执行侧探针②（非 http 链接三口径 = 0/0/0）与门禁的「检查 163 个链接，断链 0 个」一致** ⇒ **两套独立算法互证。** **⇒ 这不是被要求的，是执行侧顺手做的 —— 判据纪律开始从"外部约束"变成"工作习惯"。**
- **★ 乙级条款首次实战（第 34 批）**：**两项未命中，执行侧处置全部正确** ——
  - **③「立法同族第 25 / 26 / 27 例」字面未命中（实测 0/0/0）**：**内容确在（L79/L80/L81）**，但**文件里那三条不带「立法同族」前缀**。**⇒ 判「字面未命中 · 内容命中」，原样上报、不动我的文件。** **⇒ 这是决策侧第 24 次失误：同一份文书内两种编号字面并存（第 23/24 例用「立法同族第 NN 例」，第 25/26/27 例用「第 NN 例 · 标题」）⇒ 本批已统一为「立法同族第 NN 例」。**
  - **⑥ 标题内数字 = 9 ⇒ 执行侧改判「命中（口径分歧）」**：三口径复算 —— 全数字 9 / 其中日期占 8 位 / **剔除日期与「第 N 条」后残留 = 0** ⇒ **声明要的是「无计数」，日期与条号引用不是计数**。**⇒ 该改判已批准，且它完整列出了「改了哪个读数、按什么口径」—— 这正是乙级条款要的形态：改判可以，但不能静默。**
  - **⇒ 乙级有效性的第一个证据**：**本批两项未命中未产生任何停手往返**，全部在同批内解决并留痕。
- **★ 分级停手立法（2026-09-12，决策侧提出 + 执行侧给出可机械判定分界线）**：**依据 = 用户原话「只要做的东西不会丢失，有版本管理就 OK 了」** ⇒ **停手门槛与这个标准对齐，而不是与"零瑕疵"对齐。** **判据（唯一）=「该失误会不会造成丢失 / 不可逆 / 错进版本库且无法自愈」。**
  - **甲级（必须停手）五条 —— 全部是<可判定的状态>而非形容词**：① **指纹不符**（防篡改检查，一变就意味着"有东西动过"）；② **暂存集合 ≠ 提单清单** / 清单外条目 / `Content/` 被误暂存；③ **门禁任一阶段 `EXIT != 0`**；④ **历史 / 凭据 / 不可逆类**（`--force`、`reset --hard`、历史改写、密钥入档、删除项目文件、`E:` 盘新建）；⑤ **推送后三方 SHA 不一致** / 远端计数 ≠ 0 / **克隆内容与基准不符**。
  - **乙级（不停手，自行判定 + 修正 + 同批标注口径）**：**文书里的数字 / 文字口径 / 量具歧义 / 计数不符。**
  - **⚠ 乙级授权边界（由执行侧提出并采纳）**：**乙级授权"带着口径继续跑"，不授权"改别人的文件"** —— **「修正」仅指修正自己的读数 / 口径 / 量具实现**；**遇对方文书内容缺陷 ⇒ 只报告并定位，由作者改。** **⇒ 理由：乙级要解决的是"往返次数"，不是"权限" —— 把两者混在一起，就会用"提速"的名义打开写权限边界，而那条边界是「清单外即停手」全部保护的基础。**
  - **⇒ 附带条件（必须保留）**：**乙级必须在同批报告里逐条列出「我改了哪个读数、按什么口径」** —— **否则从"自行修正"退化成"静默修正"，而静默修正正是本纪律全部条文要防的东西。**
- **决策侧本轮失误（第 13~21 次，同一根因连续复发）**：第 13 次 台账第 29/30 行行数手算错（`+13−0`/`+5−2`，实测 `+15−0`/`+7−4`）；第 14 次 **时点手打错**（写「14:16 下发」，实为 **16:16** —— **在立第 18 条① 之后约十分钟**）；第 15 次 sha16 手打错（第 6 位 `B`→`E`）；第 16 次 **同一 SHA256 补成 62 位**；第 17 次 **「标 6 项、实列 5 项」**（A/B 分段计数 6/7，实测 5/8）；第 18 次 **沿用已过期读数**（`README.md` numstat 声明 `+10/−0`，实测 `+12/−0`）；第 19 次 **重复第 16 次的 62 位错串**（**在"为打错哈希道歉"的文书里凭记忆重抄了那个哈希**）；**第 20 次 编辑锚点即内容 —— 把 `SessionCommands.md` 的 `## 安全默认` 标题连同其位置一起替换掉了**（**与同族第 9 例「修复被静默删除的 CHANGELOG 章节标题」完全相同**；**已用字节导出法从 `b2c3dcf` 取回原文核对 ⇒ 该节为空标题、无正文丢失 ⇒ 已复原，结构核验 10/10 标题各命中恰 1 次**）。**⇒ 共同根因：写进提单的是"我认为的数"，不是"取数输出"。** **⇒ 后果全部停在零副作用处（执行侧三次停手拦截 + 本节初稿自查一次），无一条进入版本库。**
  **⇒ 第 21 次 = 第 20 次的同一动作再次发生（登记第 24 例时，`edit` 的 `old_string` 用了下一条目的标题 ⇒ 该标题被替换掉、正文被并进上一条）** —— **两分钟内同一个错误犯两次** ⇒ **由此得出可执行纪律（已写入 `SessionCommands.md` §判据与文书纪律）**：**用标题行做 `old_string` 时，`new_string` 必须以同一标题结尾（锚点即内容，替换必须自含锚点）**；**且每次编辑后必须回读被编辑行的全长**（L74 长度 1171 字符即为异常信号，正常条目约 300–600 字符）。
  **⇒ 第 22 次 = 链接形态自伤（连触三次）**：第 33 批写文书时，我在「描述门禁把行内代码当链接」这一条目里**亲手写出了该链接形态** ⇒ **门禁连判红两次**（`30/27/1/2 EXIT=1`）；改写为文字描述后转绿。**⇒ 荒诞之处在于：解释这个坑的字，自己掉进了这个坑。**
  **⇒ 第 23 次 = 提单里的数字不带量具**：我写「SessionCommands 含『锚点即内容』= 1」**但未声明这个"1"怎么数** ⇒ 执行侧实测三种口径（子串 3 / 标题 0 / 条文 1）⇒ **判「不可判定」并停手** ⇒ **一次 5 分钟的往返，起因只是一个没有量具的数字。** **⇒ 这正是分级停手立法要消除的那一类，也是第 21 条「量具命名唯一化」追加款的来源。**
  **⇒ 第 26 次 = 第 20 次的同一动作第三次发生（登记第 29 例时，`edit` 的 `old_string` 又用了下一条目的标题 ⇒ 该标题被替换掉、正文并入上一条）** —— **同一错误在 40 分钟内犯了三次。**
  **⇒ ⇒ 结论：「锚点即内容」写成纪律不管用（我三次都"知道"这条纪律）。必须升级为<可机械检查>的形式**：
```
  插入型编辑的机械规则（强制，第 26 次后升级）：
    new_string 必须**原样包含** old_string 的全文
    ⇒ 等价检查： new_string.Contains(old_string) == True
    ⇒ 两种安全形态：(a) 锚点置末（在锚点后追加）(b) 锚点置首（在锚点前插入）
    ⇒ 编辑前必须先说出这个断言；不成立就不许提交该次编辑
  理由 = 前三次全是"我以为我记得"，而该断言是可以在动手前**逐字符比对**的，
         不需要依赖记忆或自觉 —— 这正是本日全部立法反复得出的同一条经验：
         **凡是靠"记得"的，都会在第三次失效；凡是靠"可比对"的，第一次就能拦住。**
  **⇒ 第 27 次 = 第四次吃掉标签（登记第 30 例时又发生）** ⇒ **⇒ ★ 关键发现：那条"可机械检查的规则"本身没有失效，失效的是"我没有执行它"** —— **我立了 `Contains` 断言，却在这一次编辑前没有做这个断言。** **⇒ ⇒ 由此得出本日最深的一条方法论**：
```
  承诺式纪律（"编辑前先做断言"）仍然依赖执行者的自觉 ⇒ **仍然会在第四次失效**。
  ⇒ **唯一可靠的形态是「编辑后自动校验」**：把检查放到编辑之后、由脚本执行，
     使"忘记检查"这件事在物理上不可能发生（校验不过 ⇒ 当场回滚该次编辑）。
  ⇒ 一句话：**把纪律放在人的前面，不如放在编辑的后面。**
  **⇒ 第 28 次 = 条文归属错位（第 37 批被执行侧探针⑤查出）**：**第 21 条已有「追加款一 / 二 / 三」**，**而我新写的三款自称「追加款四 / 五 / 六」，却追加到了<第 20 条那一行的末尾>**；**而第 20 条自己已有 ④⑤⑥** ⇒ **两套编号在同一行并存** ⇒ **读者按"第 21 条追加款四"去查，会查到第 20 条的行里。** **⇒ 执行侧按我的字面口径测第 21 条 ⇒ 未命中，并原样上报、未替我把口径圆过去 —— 这是正确执行口径的范例。** **⇒ 修正（第 38 批）**：**把「追加款四/五/六」整段从第 20 条行迁入第 21 条行**（**与已在那里的 一/二/三 合并为一~六**），**第 20 条只保留 ④⑤⑥**；**迁移用脚本完成（避免手抄长文本），迁移后核验两行均以 ` |` 正常闭合、第 21 条含 追加款一~七、第 20 条含 ④⑤⑥ 且不含「追加款」。**
```
```
- **★ 立法有效性第二个因果证据：「只增不改」断言首跑命中**：口径⑤ 条件② 放宽为「存在活跃认领」后，**执行侧主动补加严** —— `sessions/**` 的 ` M` 须 ①工作区行数 ≥ HEAD 行数 ②前 K 行（K = HEAD 行数）逐行相同，**违反 ⇒ 停手**。**首跑实测**：`session-20260830-001.md` **HEAD 638 行 → 工作区 646 行（纯追加）+ 前 638 行逐行相同 ⇒ True**。**⇒ 该断言不依赖 taskId ↔ sessionId 映射，故不重蹈「不可判定 ⇒ 必然误停」** —— **加严与可判定性不冲突的范例：真信号不是"谁写的"，而是"记录有没有被缩短或改写"。**
- **口径⑤ 条件② 修订（决策侧，因实测撞到而改）**：**不再要求 sessionId 与 taskId 精确对应**，退化为「**根 `STATUS.json` 中存在至少一个 `status = in_progress` 且 `claimedBy` 非空的任务**」。**理由**：会话记录是**追加式工作台账**，**认领会续期**（如 `M01-T001` 的 `claimedBy` 由 `session-20260830-001` 续期为 `session-20260906-001`），**旧会话名的记录仍被继续追加** ⇒ **精确映射不可判定 ⇒ 必然误停**。**⚠ 边界不放宽**：**无活跃认领时，`sessions/**` 的改动仍一律停手。**
...
- **【立项】M00-T007 判据工具「核对执行器」任务包已建（2026-09-12）** —— 用户已批准立项；任务包 = 里程碑详规 `execution/M00/T007-JudgementTool.md` + active 五件套 `execution/active/M00-T007/`（`TASK.md` / `ALLOWLIST.txt` / `INPUTS.md` / `CHECKS.md` / `STATUS.json`）+ 根 `.trae/execution/active/STATUS.json` 登记为 **`ready`**（**成为可认领任务**，本项目此前**无 `ready` 任务**）。
  **白名单只有 2 个工具文件**：`.trae/tools/verify-claims.py`（新建）+ `.trae/tools/README.md`（新增索引行）。
  **⇒ 明确禁止执行侧改动**：**唯一 Skill（锁定级）**、**`AGENTS.md`**、**`dashboard/check-integrity.py`（冻结中）**、**一切工程资产**。**挂钩由决策侧在 TC 全绿后亲自执行**（规格 §8：**不验收自己设计的东西**；且「TC 全绿方可挂钩」是规格 §7 的硬门禁）。
  **验收判据 = 规格 §7 的 TC-01 ~ TC-13 全绿**（含新增 **TC-13** 报告段非空）；**任一不绿 ⇒ 挂钩不做**。
  **⇒ 本任务包体现「用户关切」的落地方式**：用户原话「**后面可能换人的，我怕到别人那里就忘了**」⇒ **不靠人记，靠 ① 工具自动摆在眼前 ② 机器不放行**（规格 §2）。
- **【两条新可归因证据】第 18 例立法当轮即生效 + 第 16 例实际价值（2026-09-12）** ——
  **① 第 15 条「报告义务不可被短路」当轮施工、当轮产生行为改进**：**同一现场**，**上一轮报告段静默（0 条）⇒ 本轮打印 3 条全报**；其中 **`BP_VRCharacter.uasset` 首次获得指纹报告**（此前被控制流短路吞掉）。⇒ **第 18 例立法可归因于当轮行为改变**，与第 21 提交「零事故且可归因」并列。
  **② 第 14 例口径乙复用 ⇒ 三项受限量具读数一次全中**（§4.3 = **15**、§4.4 = **7**、TC = **13**）⇒ **决策侧内容侧 100% 正确，先前两次偏差 100% 来自量具范围未限定**；且**全文读数复现**（§4.3 全文 = 17、§4.4 全文 = **19**）⇒ 范围缺陷被完整定位。
  **③ 执行侧本轮自查新缺陷 = 0 条**（脚本一次跑通：无解析错误、无静默、无量纲错、无显示层漏行）⇒ **同日第二次零事故回合**。
  **④ 第 16 例「跨文件探针须同时报另一文件候选数」体现实际价值**：探针 ⑦⑨ 在**另一文件亦有候选** ⇒ **若不报两文件候选数，即可能把归属判到错的文件上**。
- **【立法同族第 18 例 · 单列】报告义务被控制流短路 ⇒ 静默（2026-09-12）—— 本族第三种失效形态** ——
  **现象**：执行侧第 25 提交脚本的「**报告项（不阻断，逐条报指纹）**」段**输出为空**，**而当时确有 2 个应报项**（`环首刀直版.uasset` 口径③、`L_SkeletonTest.umap` 口径④）。
  **根因**：**控制流短路** —— 它把「三个资产必须未暂存」的跳过判据（`$assets`）**放在报告分支之前**，而该集合**同时含两个应报项** ⇒ **两者在到达报告分支前就被 `continue` 跳过** ⇒ **报告义务被吞掉 ⇒ 静默**。
  **⚠ 为何单列（不问"是否同第 17 例"）：** **前 17 例全部是"判据给出了错误结论"（假红 / 假绿 / 过松 / 过严）；本例是"判据根本没有给出结论"** —— **既非假红也非假绿，是「空」**。**偏离的不是"判得对不对"，而是"有没有判"** ⇒ **机制与表现均不同，单列。**
  **⚠ 本轮最要紧的一条**：**「条目计数仍正确（8）⇒ **计数判据在结构上无法发现它**」** —— **凡"正确的数"掩盖"缺失的话"，一切计数类判据都失效。** ⇒ **已写入工具规格第 15 条。**
  **⇒ 立法**：**① 报告项的产出路径必须与任何 `continue` / `skip` / `return` / 异常分支结构性解耦**；**② 必须另有一条独立的「报告段非空（当存在应报项时）」断言**；**③ 测试用例 TC-13 覆盖此场景。**
  **⇒ 处置正确**：执行侧**本轮即时补报**（未静默过夜）、**主动上报而非藏起来**、**自己定位根因**。**这是本日质量最高的一次自查。**
  **⇒ 附带（同族第 15 例，不另立）**：`push` 的 ref 更新行未打印（正则 `\.\.\s` 与实际 `97a15e9..9e7924e` 不匹配）⇒ 显示层漏行，靠 `exit=0` + 三方同 SHA 佐证；另有 2 次 PS 解析错误，**均零副作用、未执行任何语句**。
- **【立法同族第 19 例 · 方向与第 18 例相反】取数失败被伪造为一个数值（2026-09-12）—— 本族第三种失效形态的镜像** ——
  **现象**：执行侧第 29 批第一次尝试中，结构段三项读数**全部异常**（`##` = **0**、围栏数 = **行数**）⇒ **其自建断言 `structOk=False` 抛错终止**。
  **根因**：量具函数里把全局变量写成 **`$GLOB:enc8`**（应为 `$global:`）⇒ PowerShell 解析为「驱动器变量」= `null` ⇒ `ReadAllText($path, $null)` 抛 `ArgumentNullException` ⇒ **`$t` 为 null ⇒ `##` 被"算成" 0**；**围栏因 `StartsWith($null→"")` 恒真 ⇒ 围栏数被"算成"行数**。
  **⚠ 最要紧的一句**：**「取数失败」被显示成一个数值（`0`），而不是「未取到数」** —— **`0` 是一个"看起来很合理"的值，没有人会去质疑它。**
  **⇒ 单列理由（与第 18 例的关系）**：第 18 例 = **该出声却无声**（报告段静默 ⇒ 输出为空）；**本例 = 该无声却发了一声假音**（取数失败却输出 `0`）。**两者是同一枚硬币的两面，方向完全相反，必须分列。**
  **⇒ 立法（已写入工具规格 §4.3 第 16 条 + §7 TC-14）**：**任何量具 / 取数函数返回 `null`、空串或空集合 ⇒ 必须抛错**；**严禁把取数失败表示成 `0` / 空串 / 空集合**；**并配套断言：报告段与结构段的每一个数值都必须可追溯至一次「非空取数」。**
  **⇒ 处置评价**：**① 停手在 `git add` 之前；② 零副作用；③ 是自己的断言拦住的（没有变成假绿）；④ 修复为参数传值 + 四处取数函数内建非空断言 ⇒ 重跑六项 3/3 全中。** **这是本日质量最高的"自查 + 自修"。**
  **⇒ 立法当批见效**：第 29 批重跑中，**`-f` 纪律 100% 执行**（每个参数先单列算好）⇒ **本批零格式 / 解析错误**（前一轮同日 3 次）。**⇒ 裁 3 的立法可归因。**
- **【判据失误家族 · 决策侧第 9 次】我数错了批次号（2026-09-12）** —— **决策侧把 `d49e9a8` 标为「第二十九提交」，实际是第 28 次提交**（台账行序：24=`97a15e9` / 25=`9e7924e` / 26=`fad327a` / 27=`520a5a4` / **28=`d49e9a8`**）⇒ **跳号，off-by-one**。
  **⇒ 且由此在提单内部制造了三处互相矛盾的数字**（`⑫ 台账数字行 = 27` / `常规核验 台账补记第 29 行` / `提交信息 补记第 27 行`）—— **执行侧在写报告时发现并正确判定「实测唯一解 = 补记第 27 行成立、第 29 行本批无法成立」，未停手、按第 14 例处置并上报。**
  **⇒ 这是「基准数字零转录」立法后的第 1 次违反（同日第 3 次数值失误：第 6 次 `9` → 第 7 次 `119` → 第 9 次「第 29 批」）。**
  **⇒ 执行侧的处置评价**：**① 识别滞后属实（预检只断言了 ⑫ = 27，未做交叉一致性检查）；② 但它在写报告时自曝滞后、不粉饰；③ 判定"可由实测唯一确定"的依据完整（第 28 批未产生提交 ⇒ 本提交应记为第 28 行、由下一批补记）。**
  **⇒ 立法（决策侧自控，追加）**：**提单内所有数字必须交叉一致** —— 同一份提单里出现两个互相矛盾的数字，**本身就是判据缺陷**；**决策侧在下发前必须跑一遍「内部数字一致性检查」。**
- **【规则 23 口径澄清】「本机路径凭据」的边界写进正文（2026-09-12）** —— 起因：**执行侧在提单之外主动补跑规则 23 安全扫描**，发现 `M00-T007` 任务包的 `INPUTS.md` / `CHECKS.md` **各含 1 处解释器绝对路径**（`D:\App\trae\Traedata\...\python.exe`）并**提请裁定**（**它未自行定性**）。
  **澄清定义**：「本机路径凭据」= **可用于获得访问权的路径/账号信息**（密钥文件路径及其内容、共享目录凭据、带 token 的 URL）；**不含「工具链可执行文件的绝对路径」**。
  **判断标准**：**该路径泄露后能否被用来取得访问权 ⇒ 能则禁，不能则留。**
  **⇒ 裁定：保留。** 理由：① **非凭据**；② **是可复现性依赖**（**没有它，换会话/换人后连门禁都跑不起来** —— 正是用户「**我怕到别人那里就忘了**」要防的情形）；③ **既有惯例一致**（规则 28 本身即含 `D:\App\trae\ue-mcp\1.0.87\...`）。
  **⇒ 立法价值**：**一条边界模糊的规则会让下一个执行者要么误停手、要么自行猜测** ⇒ **模糊边界本身就是事故源**。
- **【判据失误家族 · 全新形态】「手里有正确读数，落笔时改了」—— 同日连犯 2 次（2026-09-12）** ——
  **决策侧第 6 次**：**提单写 `activeTasks = 9`，实测 8** —— 而**决策侧在提单前一轮的实测输出里已正确取得 `tasks=8`**。
  **决策侧第 7 次**：**提单写 `AGENTS.md = 119 行`，实测 118** —— **决策侧此前的同级实测基准本就是 118**，且本次编辑是**同行替换、行数不变**。
  **⇒ 两次的共同点：不是"测不出"，而是"测对了、写错了"。**
  **⇒ 这是本族此前 18 例中从未单独出现过的形态**：假红 / 假绿 / 过松 / 过严 / 静默**都发生在判据或实现内部**；**这两次发生在「人把读数搬到提单上」这一步**。
  **⇒ 立法（决策侧自控）**：**「基准数字零转录」** —— **提单中的每个数字必须与实测输出逐字符一致**；**禁止手打、禁止心算、禁止"应该是"**；**同一批内任意两个基准数字互相矛盾 ⇒ 视为判据缺陷，相关方停手回报。**
  **⇒ 执行侧的处置正确**：**两次都未折算为通过、未猜、未代写，而是停手取证并请裁定。**
  **⇒ 同族第 8 例（决策侧，同日）：不可满足探针在提单前被自查拦住（2026-09-12）。** 决策侧在第 28 批提单中要求执行侧核验字面串「**决策侧第 6 次**」，而 **CHANGELOG 当时写的是「第 6 次」** ⇒ **该判据在磁盘上不存在候选**。**⇒ 依「可满足性前置」，若照此下发，执行侧会判「判据不成立」—— 它不会误判为「未通过」，但会白白停手一轮。** **⇒ 本次由决策侧在提单前的自查中命中并当场改写为字面一致 ⇒ 未产生任何时间损失。** **⇒ 这是「可满足性前置」立法后第一次在提单离开决策侧之前拦住错误。**
- **【第三例同源工具故障 ⇒ 立法】`-f` 参数禁止内联表达式（2026-09-12）** —— 执行侧本轮取证命令中有一行 `Write-Output ("..." -f $t.Count,$t.Count-1182,...)`，**在 `-f` 的参数列表里写了内联减法** ⇒ `FormatError: Index (zero based) must be ...`，**该行读数未打印**；**它随后重新实测补齐了该读数**。
  **⇒ 这是执行侧同日第 3 次 `-f` 同源故障**（前两次：解析错误；本次：FormatError），**三次全部零副作用、数据未受损**。
  **⇒ 立法（采纳执行侧的提议，它只提请不自行立法）**：**`-f` 的所有参数必须是纯变量或字面量，禁止内联运算 / 赋值 / `switch`**；**需要计算 ⇒ 先单列一行算好再传**。
  **⇒ 立法价值**：**3 次同源故障用一个纯机械约束即可结构性消除** —— 属于「把纪律交给语法，而不是交给记性」。
- **【第 14 例立法首次实战拦住假红 · 可归因证据】（2026-09-12）** —— 执行侧核验 `AGENTS.md` 规则编号时，**口径甲（字面"无重号"）判「不通过」**，**若照此执行，它会去改一个完全正确的文件**；**它按第 14 例「量具定义显式声明」处理**：先定位重号真身（= `## 必读` 的 1-4 项，**并非规则编号重复**）、判定**判据定义有缺陷而内容无缺陷**、**未据此判「未通过」**，并按新口径乙（编号集合 = {1..32}，排除〈必读〉列表）重测 ⇒ **通过**。
  **⇒ 这是第 14 例立法后第一次实战拦住一次假红，且可归因于该条立法本身** —— 与第 21 提交「零事故且可归因」并列，构成**立法有效性的第二条可归因证据**。
  **⇒ 决策侧据实自曝**：**该探针 ⑭ 由决策侧设计，未声明量具定义 ⇒ 缺陷在决策侧，不在执行侧**（决策侧当日第 4 次判据自身失误）。
- **【用户裁定「就这么定」】推送决策正文 + 副本与可追溯纪律 + 唯一 Skill 挂钩（2026-09-12）** —— 用户裁定：「**就这么定**」。用户同时裁定**删除远端废弃分支 `main`**，并问「其他人一般也是只留一个吗」⇒ **答：不是「只能留一个」，而是「不许留没人认领的」** —— 大项目常有主线 / 开发线 / 功能线 / 发布线 / 部署线多分支，**关键在每个都有主、有用途、有归宿**。
  **① 「何时推」唯一权威正文**：`governance/SessionCommands.md` §推送 新增 **〈何时推送（推送决策）〉** —— 四行授权表（治理文档类 ⇒ 决策侧自行判断可直接推 / 工程资产与任何删除·范围变更·验收结论 ⇒ 必须先问用户 / 清单外 `M` 类·门禁非 0·含密钥·含生成目录·文件正在被写入 ⇒ 一律不推）+ 五条放行前提（门禁前后各一次 `EXIT=0`、清单逐条点名、暂存集严格等于清单、三方同 SHA、两层报告）。**目的：新决策模型无需询问即可自行判断。**
  **② 副本与可追溯纪律**：`AGENTS.md` 新增 **§副本与可追溯（规则 29–32）** —— **29** 唯一权威副本 = 项目工作区 + git 远端，**禁止本地多处备份**；**30** **不允许长期存在无人认领的副本 / 分支 / 归档**（"留着以后再说"本身就是下一次"不知道哪份最新"的种子）；**31** 判定冗余准则（**存在逐字节等价副本 ⇒ 冗余可删；无副本且承载决策或事故记录 ⇒ 保留**）；**32** 远端分支纪律（主线外每个分支必须有用途与维护者）。
  **③ 磁盘规则据实修订**：`E:` 用途原文含「**备份**」、示例含「**旧版本备份**」 ⇒ **与用户本次裁定直接冲突，已移除「备份」**（只留「大文件下载、低频工具、归档」）。
  **④ 唯一 Skill 挂钩（锁定文件，用户已专项批准）**：模式路由后新增两条**只链接不复制**的指引（规则 19）—— 指向〈何时推送〉与 `AGENTS.md` §副本与可追溯。
  **⑤ 远端分支实测 —— 据实纠正决策侧自己的推断**：GitHub REST 实测 **`"default_branch":"master"`** ⇒ **默认分支正确，外人看到的就是最新**；决策侧先前据**本地陈旧 `origin/HEAD -> main`** 推断"默认分支是 main、外人看到空仓库" ⇒ **属误判，已据实更正**（先查证再断言，未把推断写入正文）。**真实问题** = `main` 是 2026-08-09 的 `Initial commit`、**与 master 分叉**（1 个提交不在 master 历史内）⇒ 用户裁定**删除**。
  **⑥ 判据工具规格新增禁用写法**：`execution/CR-20260912-001-judgement-tool.md` §4.4 新增第 7 条 —— **禁止用「截断文本」做归属判定**（依据：执行侧用被截断的列表页标题做前缀比对 ⇒ 归属 = False，而真实归属由 API `head_sha == 97a15e9` 直接证明 ⇒ **内容对、判据方法错**，同第 17 例家族）。
...
| 44 | `1b02ff1` | 通读产品规格修 3 处矛盾（护甲字段 / 平滑移动定性 / REQ-011 冲突）+ 立 §3.21 权威定性句纪律 + 立 §3.22 凭证标签纪律 + 修 §3.20/§3.21 章序 + §3.18 补跨文件引用条款 + 新增 plugin-scout 取证入库（8 files，+796−9） | 结论待取（403，配额；本批连 run id 均未取到） | 同左 |
