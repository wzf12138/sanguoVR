...
## Live Coding 补丁与二进制（2026-09-10 入库，CR-20260909-001 E 节）

- **事实**：`live_coding` 热编译只把 C++ 方法体补丁写进**当前编辑器进程的内存**。进程退出后补丁消失，症状会原样复现。
- **只有走 UBT 完整编译并写入 `Binaries/` 的改动才跨进程存活。** 二者不可互相替代。
- **实证**：M01-T001 抓取路径崩溃（`IVRGripInterface::DenyGripping` 直调）首次用 live_coding 修好并通过验证，编辑器重启后崩溃复现；改走 UBT 完整编译后消除（CR-20260909-001 B/E 节）。
- **交付纪律**：任何"修好并验证通过"的 C++ 改动，交付前必须写清它**怎么进二进制的**——"已 UBT 完整编译"或"仅 live_coding（进程内存态，重启即失效）"。只做了 live_coding 的改动一律标记**未持久化**，不得计入已完成，须补 UBT 编译后重新验证。
- **顺带红线（同一事故族，2026-09-10 立）**：禁止"读取截断 → 回写"的写入路径。处理 `STATUS.json` 等含大字段的文件必须整文件一次写入、写完立即 `JSON.parse` 回读校验，失败即恢复备份并报告（CR-20260909-001 D-1）。

## M01-T001 握持体系：VRE 完整方案 + 一次性校准（2026-09-10，依据待核）

- **架构方向**：武器握持体系走 **VRE（VRExpansionPlugin）完整方案**（不自研抓取层），方法论改为"**对照 VRE 官方约定实现、一次性校准**"。
- **依据状态**：exec-m01-t001 2026-09-10 19:01 汇报称该决策"已获用户批准"，引用信 `msg-20260910-111152`。**该信不在 decision 收件箱**（疑在 manager 侧）。**2026-09-12 更新**：执行侧已于 `msg-20260910-235545` §六 **原文重投该信要点**（"原信 2026-09-10 11:11:52 我投 decision"），**依据已坐实到"原文重投"级别**；**原始投递缺失的原因未查明，不推测**（据实登记为 AgentHub 信箱可靠性事件：**"已投递"与"已收到"不是同一件事**）。
- **用户直接确认的三项校准需求**（2026-09-10，经指挥台 ask_user_question 逐项取得，与 M01-T001 四项校准同批）：
  1. 握持点贴合（握持点与刀柄对齐）；
  2. **剑/刀竖握**（pitch 90，用户明确要竖握姿态）；
  3. 删除旧握持点发光标记（GripPulse ×2）；
  4. 手指握拳姿势：**分级交互**——grip = 三指弯曲，trigger = 加食指（用户确认）。
- **实施顺序**（exec-m01-t001 提报，decision 认可）：① V2 诊断日志定位断点 → ② 握持点改 VRE 标准 `VRGripSocket` → ③ 双手持刀验证 → ④ 手指弯曲（VRE `VRHandPose`）→ ⑤ 参数收敛后解锁落盘 → ⑥ TC-01~06 全量取证（标 VR Preview 层级）→ ⑦ CR-20260905-001 ③④ 回填与任务报告。
- **配套纪律**：live coding 补丁不跨进程存活（见上节"Live Coding 补丁与二进制"）；本轮多项改动为内存态，**编辑器重启即丢失**，落盘前不得重启进程。
- **⚠ 握持姿态由什么决定（2026-09-10 实测更正，推翻决策侧此前判断）**：
  - **结论**：**`VRGripSocket` 的相对变换确实决定 `AttachmentGrip` 的握持姿态**，传导链**无隐藏覆盖**。
  - **实证（三组 GripDetail `RelTrans` 与 socket 变换精确互逆）**：`pitch70` → `(-70,0,0)`；`pitch90` → `(-46,0,9)｜(-90,0,0)`；`roll ∓90` → `(0,0,±90)`。
  - **此前"改 socket 刀不变"的真因（据实更正）**：**不是 socket 无效，是手柄坐标系假设错误** —— PICO 手柄**自然握持时手柄轴接近竖直（手 X ≈ 世界向上）**，而项目曾按"手 X = 水平前方"推算角度 ⇒ **全部试错方向系统性偏离**，表现为"改了没反应"。
  - **修正后结果**：socket `pitch = -160`，**用户实测确认"刀方向不错"**（刀身朝上举）。
  - **决策侧据此更正**：此前给执行侧的"**socket 不决定姿态**"提示**作废** —— 该提示建立在错误的坐标系假设之上。**这是决策侧基于假设而非实测下判断的一次失误，据实记录。**
  - **⚠ 证据保全（硬约束）**：**原始 GripDetail 日志摘录是上述结论的唯一证据**（三组互逆数据的唯一来源）⇒ **不得删除、不得覆盖、不得以转述替代**。凡引用本条结论处，必须能回溯到该日志。
  - **方法论（已入判据族）**：**坐标系假设必须先实测确认，再用于推算角度**；**"改了没反应"时先怀疑参照系假设，而非先怀疑目标属性无效**。
