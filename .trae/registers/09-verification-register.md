...
| V-007 | NTFS ACL 应用与恢复 | M00-T001 | 系统权限 | 已验证（2026-08-26） | 用户在管理员 PowerShell 中执行 `D:\AWork\TraeAdmin\VRSanguoYanWuchang\Set-TraeGovernanceAcl.ps1 -Mode Apply -Confirm:$false` 完成 ACL 应用。`Get-Acl` 核验：治理目录 `D:\AWork\Unreal\Project\VRSanguoYanWuchang\.trae` 与管理员脚本目录 `D:\AWork\TraeAdmin\VRSanguoYanWuchang` 均 `Protected: True`（继承断开）、`IsInherited: False`（显式设置）。治理目录 Users 组权限从 Modify 降级为 ReadAndExecute；管理员脚本目录 Users 组完全移除。DESKTOP-HEUDI0T\PC 保留 Modify 权限保证后续 AI 维护。完整 ACL 输出与执行回放见 `execution/reports/tasks/M00-T001-acl-2026-08-26.md` |
...
| V-010 | `Saved/Screenshots/` 目录<来源混合>，不得作为验收证据 | 全项目 | 文件 | 已裁定（2026-09-13） | **裁定（指挥台，2026-09-13）**：该目录为**多会话共享的过程性输出目录**，共存 63 张截图，**两个相关会话（`exec-m01-t001`、`session-20260908-002`）均如实答「不知道归属」，指挥台采信其诚实而不追究**。**⇒ 口径**：① **`Saved/Screenshots/` 承认来源混合，按现状保持，不逐张归因、不再追责**；② **正式验收证据一律走 `Saved/Evidence/{任务号}/`**，不得再混入该目录；③ 该目录受 `.gitignore` 忽略，**不构成版本控制覆盖**，故不作为任何验收依据 |
