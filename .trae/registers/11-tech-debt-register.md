...
| TD-010 | 编辑器自动化工具链变更：安装 UEBridgeMCP（GPL-3.0），移除未编译通过的 UnrealMCP（MIT） | Plugins/UEBridgeMCP（已退役→UEBridgeMCP.disabled）、VRSanguoYanWuchang.uproject | UE5.6 无官方 MCP（官方仅 5.8）；UnrealMCP 在 5.6 编译失败，UEBridgeMCP 5.6 编译零改动通过 | 编辑器可通过 HTTP 8080 被 AI 自动化操作（407 工具）；GPL-3.0 许可证对商业分发有传染风险 | **已偿还（2026-08-31，M02-PREP-002）**：按用户批准方案 A + manager 重试授权，迁移至 db-lyon/ue-mcp **v1.0.87**（MIT）完成——npm v1.3.0 在 UE5.6 系统性不兼容（61 编译错误，静态探针证实 5.7+ API 未门控），横向探测 6 个历史版本后选定 v1.0.87（13 个问题符号仅 2 处且已门控），1 行版本门控补丁（AssetHandlers_Import.cpp `SetSourceString` 3参→2参，5.7+ 才用 3 参）后 UBT 编译 `Result: Succeeded`；TC-02 蓝图测试副本写读验证读回一致；UEBridgeMCP 退役（目录改名 `UEBridgeMCP.disabled` 保留审计 + `.uproject` 移除条目），重启无缺失模块错误，L_SkeletonTest 冒烟正常。**GPL-3.0 传染风险随退役消除**。维护注：桥插件含本地 1 行补丁，未来执行 `ue-mcp deploy/update` 会覆盖插件源，需重打补丁或向上游提交。证据：`Saved/Evidence/M02-PREP-002/`。历史尝试记录：v1.3.0 首轮失败（blocked）→ v1.3.1-beta.2 静态探针判定仍不兼容（未浪费编译）→ v1.0.87 成功 | 中 | resolved |
