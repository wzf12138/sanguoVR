...
| TD-013 | M01-T001 使用 VRE 内置 GripObject 做抓取功能验证，未按 systems/02 IGrabbable 接口封装 | Content/VRSanguo/VR/BP_VRCharacter（待创建）| M01-T001 决策：功能验证优先，Route B（自研 IGrabbable 接口）投入产出比不合理且蓝图调用具体类可能触犯 03-blueprint-cpp-boundary-standard | GripObject 是 VRE 内部实现，不经过项目 IGrabbable 接口（TryGrab/Release/SwitchHand/GetGrabPoints）；武器抓取逻辑与项目交互系统脱钩 | M01 后续任务（T002/T003）或 M02 按 `systems/02-interaction-and-weapon-system.md` 接口标准重构，将 GripObject 调用封装进 IGrabbable 桥接层 | 中 | open |
