...
- 是否需要管理员权限/外部凭据/联网：否（.trae/rules/project_rules.md 由 PC 用户 Modify 权限可删，前日已 Apply ACL 保护）。

- 是否包含删除、迁移、批量重命名或格式升级：
  - 删除 1 个文件（.trae/rules/project_rules.md）
...
- 文档逐份回读：
  - AGENTS.md（与 project_rules.md 原文逐条对比，确保零字差异）
...
- 实施任务编号：当前会话 session-20260825-002（无 active 任务，使用本 CR 作为执行边界，按 project_rules.md 第 19c 条「用户本次明确批准的变更」授权白名单）
