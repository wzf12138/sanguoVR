...
| 2 | 地面 **现行 120×120m**（用户 2026-09-11 确认满意；**尺寸非最终，可能调整**）· 围墙 4m 高<br>⚠ 原文「20x20m」为 v1 规划历史值，**验收以关卡实测为准** | 编辑器测量工具 |
...
> 依据 `governance/TestSpecification.md` 设计（关卡创建 ≥2 条：加载 + PIE 运行）。本表为验收清单中可复核项的正式化；验收清单其余目测项仍然有效。

| 用例ID | 测试目标 | 输入/前置条件 | 预期输出 | 证据类型 |
|--------|---------|-------------|---------|---------|
| TC-01 | 关卡加载 | 编辑器打开 `L_Prototype_1v1_v2.umap` | 无报错、无丢失资产警告 | [截图] |
| TC-02 | NavMesh 覆盖与可达 | 显示 Navigation，从两出生点寻路至高台 | 全覆盖绿色，高台可达，掩体边距 ≥0.5m | [截图] |
| TC-03 | PIE 帧率达标 | PIE 运行 + stat fps | ≥90fps，无渲染报错 | [stat 截图] |
| TC-04 | 光照烘焙结果 | SkyAtmosphere/SkyLight/Fog 配置完成后烘焙 | 天空雾效正确，无漏光黑斑 | [截图] |
