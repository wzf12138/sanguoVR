...
本任务涉及以下三类内容时，必须在**设计/实现之前**逐项向用户确认，**不得自行定义、不得假设、不得把多项合并成一道题**：

- **形体/造型**：结构形制、构件样式、外观选择（如"这栋建筑长什么样"）。
- **尺度/数量**：场地尺寸、构件尺寸与数量、间距、步高、净空等一切数值。
- **玩法/机制定义**：某个场景元素"用来干什么"——包括玩法用途、胜负条件、奖励/彩头、角色分工、观众与指挥职能等。

**执行规则**：

1. 开工前建立"待确认清单"，逐项写明：问题、候选方案（2~3 个）、推荐项与推荐理由（用产品语言，不写 C++ 术语）。
2. **一项一个问题**，逐项发给用户确认；禁止把多个待确认项塞进同一个问题里一次问完。
3. 用户答复前，该项**不得**进入设计、不得写入任何报告、不得登记为"锚点/已定方案"。
4. **禁止引申**：用户确认了 A（例如"要这个建筑"），不等于确认了 B（例如"这个建筑承载某种玩法"）。B 必须单独问。
5. 报告与会话记录中凡出现"用户拍板/用户确认/用户批准"，必须**附问题原文与用户原话**；无原文引用的"用户确认"表述，审核直接判不合格。
6. 未确认项一律写"待用户确认"，不得以"通常做法"代替确认。

> 依据：AGENTS.md 规则 7（方案先确认）、规则 8（翻译成产品语言）、规则 9（范围变更须批准）、规则 10（不明确细节必须列选项确认）。2026-09-10 立（缘起：M01-T005 竞技场两条机制锚点被自行引申并写成"用户拍板"，用户指出从未确认）。
