...
| 链接计数 | 实测链接数与 `integrity.yaml` 声明一致 | **已核对**：门禁输出「检查 **163** 个链接，断链 **0** 个」，与 `integrity.yaml` 的 `checked: 163` / `broken: 0` 一致。**遗留**：`integrity.yaml` 缺 `scope` 字段，门禁按「无法判定口径」给 **warn**（不阻断）——已登记为后续加固项。 |
