...
### 9.7 裁定回写四：概念分离 ——「具体场地尺寸」与「按人数计的活动区规格」（用户裁定 ①）

**用户裁定（2026-09-12）**：在 **①概念分离 / ②三档整体上调 / ③整表作废** 三选项中，**用户明确选择 ①**。

> **本框为唯一权威表述，其他位置只链接：**
> **「某个具体场地的尺寸」与「打 N 人对 N 人时竞技场该多大」是两个不同概念，不得混于同一张表。**
> **① 具体场地尺寸** → 见 `knowledge/Production/LevelDesignSpec.md`「**演武场（1v1 主场景）尺寸标准**」= **`120m × 120m`**（§9.6 已固化）。
> **② 按人数计的通用活动区规格** → **现为「待定」，没有任何有效数值**；旧三档（`3v3 默认 50×50` / `2v2 35×35` / `4v4 70×70`）**已于 2026-09-12 作废，仅存历史记录**。

**落地**：

| 文件 | 变更 |
|---|---|
| `knowledge/Production/LevelDesignSpec.md` | **「核心指标」表移除「可活动区域」一行**（该行属另一概念）；**移除不等于删除记录** —— 旧值原样移入新增小节「**活动区规格（按人数计）—— 待定**」并保留删除线与作废理由；`核心指标` 表保留其余各行（出生点间距 / 出生点距边界 / 走廊最小宽度 / 天花板高度）**继续有效**；`演武场` 小节补记「所属概念 = 具体场地尺寸」 |
| `registers/01-decision-register.md` | **DEC-014 补记**该裁定（含「概念分离」与「按人数概念现无有效数值」） |

**约束（硬）**：

1. **在重审或给出新口径之前，「按人数计」概念没有任何有效数值。**
2. **任何新关卡不得引用旧三档数字**；**也不得以「1v1 用 120×120」外推其他人数**（外推即违反规则 10）。
3. **不得再建「混合表」** —— 将来若新增尺寸类指标，**必须先声明它属于哪个概念**。

**⇒ 记录方式说明（守规则 3「历史记录不回溯改写」）**：旧三档数字**未被抹去**，而是**带删除线移入新小节的「已作废旧值」表**，与其作废理由同处。**「移除字段」与「抹掉记录」是两件事 —— 本 CR 只做前者。**

**⇒ 本节与前序的关系**：§9.3 定「尺寸待定」→ §9.6 定「1v1 = 120×120（升为规格标准）」→ **§9.7 定「两个概念分离」**。**三节共同构成现行完整口径，不得只引其一。**
