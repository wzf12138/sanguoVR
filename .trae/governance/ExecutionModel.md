# 执行模型

## 定位

执行模型实施当前认领的任务，不负责决定产品方向、扩大范围、批准结果或领取下一任务。

## 触发

以下表达进入执行模式：

- 执行当前任务
- 开始当前任务
- 按任务包实施
- 继续执行任务

## 任务认领（必须在任何文件修改之前完成）

> **硬性门禁**：未完成认领不得开始实施、不得修改任何文件。此规则对 AI 执行和用户执行同样适用。

执行模型启动时，从 `.trae/execution/active/STATUS.json`（下称**根 STATUS.json**）的 `activeTasks` 数组中选择一个 `status` 为 `ready` 的任务认领。认领**必须先检查后更新**：

1. **检查五件套**：确认 `.trae/execution/active/{taskId}/` 下存在 `TASK.md`、`ALLOWLIST.txt`、`INPUTS.md`、`CHECKS.md`、`STATUS.json` 五个文件且内容完整。
2. **冲突检测**：读取其它 `in_progress` 任务的 `ALLOWLIST.txt`，确认无独占路径重叠。共享文件（`.trae/CHANGELOG.md`、登记册、`integrity.yaml`）不视为冲突。ALLOWLIST 中所有路径相对于项目根目录 `VRSanguoYanWuchang/`。
3. **认领（写操作）**：以上两步通过后，立即更新根 STATUS.json 中对应条目的 `status` 为 `in_progress`，`claimedBy` 填写当前会话标识。格式：`"session-{YYYYMMDD}-{序号}"`，如 `"session-20260811-001"`。同时同步更新 `.trae/execution/active/{taskId}/STATUS.json`，并**在同一轮操作中**创建会话记录 `.trae/execution/sessions/{claimedBy}.md`（按"会话记录（强制）"章节模板，填写会话开始时间、认领任务、当前上下文摘要）。**会话记录未创建 = 认领未完成，不得修改任何任务文件。**
4. **认领后验证**：重新读取根 STATUS.json，确认 `status` 确实为 `in_progress`、`claimedBy` 为当前会话，且会话记录 `.trae/execution/sessions/{claimedBy}.md` 已存在。确认通过后才能进入前置门禁。

用户执行任务的认领：由 AI 监督模型代为更新根 STATUS.json（`status` 为 `in_progress`，`claimedBy` 填写 `"user-{YYYYMMDD}-{序号}"`），或提示用户确认后更新。

根 STATUS.json 是任务状态的**唯一权威**。任务目录下的 `STATUS.json` 为本地便利副本，冲突时以根 STATUS.json 为准。

### 状态更新时机（强制）

| 时机 | 操作 | 谁负责 |
|------|------|--------|
| 开始执行前 | `ready` -> `in_progress`，填写 `claimedBy` | 执行模型或 AI 监督模型 |
| 遇到阻塞时 | `in_progress` -> `blocked`，记录原因 | 执行模型 |
| 完成实施后 | `in_progress` -> `awaiting_review`，写入报告 | 执行模型 |
| 审核通过后 | `awaiting_review` -> `approved` | 审核模型/用户 |
| 审核需修改 | `awaiting_review` -> `in_progress` | 审核模型 |
| 审核拒绝 | `awaiting_review` -> `blocked` | 审核模型 |

每次状态变更必须同时更新根 STATUS.json 和任务目录 STATUS.json，并在 CHANGELOG 记录。

## 提交审核前自检（硬性门禁）

> **执行模型在将任务状态更新为 `awaiting_review` 之前，必须逐项完成以下自检。任一自检项未通过，任务必须保持 `in_progress`，不得提交审核。**

### 自检清单

1. **交付物完整性**：逐项核对 `TASK.md` 中列出的全部交付物，确认每个交付物**实际存在、非空且不是占位正文**。未产出的交付物不得以"待办"、"后续补充"、"已规划"等理由跳过。
2. **CHECKS.md 全项通过**：逐项核对 `CHECKS.md` 中"执行后"段的每一条检查项，确认全部通过且有实际证据。**对于引用生产就绪基准（LV-/BP-/CP-/AS-）的检查项，必须对照 `definition-of-done.md` §7 中对应的量化验收条件，逐一确认**。未通过的检查项不得以"待用户执行"、"后续验证"等理由豁免。
3. **禁止路径未修改**：对照 `ALLOWLIST.txt` 确认无越界修改。禁止路径（`Source/`、`Config/`、`Plugins/`、治理锁定文件等）未被修改。
4. **验证证据可定位**：每项通过的检查均有对应的可定位证据（文件路径、日志行号、编辑器截图引用等），不存在"已确认"但无证据链的声明。
5. **无遗留待办伪装**：任务报告中不得出现将未完成交付物包装为"遗留待办"、"风险"或"后续工作"的表述，除非该交付物在 `TASK.md` 中明确标注为"可选"或"条件性"（如"如时间允许"、"P2 优先级"）。
6. **会话记录完整**：`.trae/execution/sessions/{claimedBy}.md` 存在且已更新至当前进度（含执行进度、沟通记录、恢复上下文小节）。缺失或长期未更新 = 自检失败，不得提交审核。

### 证据打包（硬性要求）

> **提交 `awaiting_review` 前，必须在任务报告中填写结构化验证表。表中每条 CHECKS.md "执行后"检查项必须逐条对应，不得合并、不得省略。**

任务报告末尾必须包含以下格式的验证表：

```markdown
## 验证证据

| # | CHECKS 检查项 | 证据类型 | 状态 | 证据路径/说明 |
|---|-------------|---------|------|-------------|
| 1 | （逐字抄录 CHECKS.md 检查项原文） | 文件/截图/日志/PIE | ✅/❌/🚫 | 具体路径或说明 |
| 2 | ... | ... | ... | ... |
```

**填写规则**：

- **证据类型**：每条检查项必须在 CHECKS.md 中已标注 `[证据类型]`。执行模型按标注类型提供证据。
  - `[文件]`：提供文件路径，确认文件存在且非空。
  - `[截图]`：提供截图文件路径或编辑器 Viewport 截图引用。
  - `[日志]`：提供日志文件路径 + 关键行号或内容摘要。
  - `[PIE]`：必须由用户在 VR 设备上完成 PIE 验证。**执行模型无法自行完成 `[PIE]` 类型检查项。**
- **状态**：
  - `✅` = 已验证通过，有证据。
  - `❌` = 已验证但未通过。
  - `🚫` = 无法验证（阻塞）。
- **禁止**：证据列填写"待验证"、"后续补充"、"已确认"（无路径）等模糊表述。

### 多模态任务特殊规则

> **当任务包含 `[PIE]` 类型检查项时，执行模型必须遵守以下规则：**

1. **执行模型只能完成非 `[PIE]` 的检查项**（如 `[文件]`、`[日志]`）。
2. **所有 `[PIE]` 检查项保持 `🚫` 状态**，证据列填写"需用户在 VR 设备上完成 PIE 验证"。
3. **任务状态设为 `blocked`**（而非 `awaiting_review`），在根 STATUS.json 的 `note` 中明确列出阻塞项和所需用户操作。
4. **用户完成 PIE 验证后**，由用户或审核模型将状态更新为 `awaiting_review`（或 `approved`）。

### 自检失败处理

- 任一自检项失败 → 任务保持 `in_progress`，继续实施缺失项。
- 自检项失败但执行模型无法自行完成（如依赖用户操作、`[PIE]` 验证）→ 任务转为 `blocked`，在报告中明确列出阻塞项和所需用户操作。
- **禁止**：自检失败后仍将状态更新为 `awaiting_review`。
- **禁止**：将 `🚫`（阻塞/待用户操作）的验证表直接提交为 `awaiting_review`。

## 前置门禁

必须同时满足：

- 已认领任务，根 STATUS.json 中对应条目 `status` 为 `in_progress`。
- `.trae/execution/active/{taskId}/ALLOWLIST.txt` 至少包含一个明确路径（路径相对于项目根目录）。
-  `.trae/execution/active/{taskId}/TASK.md` 有目标、步骤、验证、停止条件、回退和报告路径。
-  `.trae/execution/active/{taskId}/INPUTS.md` 所需事实源与输入可读。
-  `.trae/execution/active/{taskId}/CHECKS.md` 验证方法与门禁可执行。
- 完整性检查无锁定文件变化。
- 与其它 `in_progress` 任务无白名单路径冲突。

任一条件不满足时停止，不得自行补任务或扩大白名单。

### 铁律启动验证（硬性门禁，在前置门禁之后、执行规则之前）

> **执行模型在开始任何文件修改之前，必须完成以下铁律验证。未通过不得开始实施。**

1. **读取项目规则**：读取根级 `AGENTS.md` 全文，确认理解以下强制规则：
   - 磁盘与下载规则：所有下载/文件写入必须指定 D: 或 E: 盘，**禁止默认写入 C 盘或用户主目录**。
   - 文件边界：临时脚本、下载和调试输出不得进入项目。
   - 认领门禁：已在任务认领步骤完成。
   - 执行规则：不把规划描述为实现，不把桌面验证描述为 PICO 真机验证。
2. **磁盘规则自检**：在创建任何文件前，确认目标路径符合磁盘规则（项目文件 → D: 项目内；**临时文件 → 项目外临时目录 `D:\AWork\TraeAdmin\VRSanguoYanWuchang\tmp\`**；大文件下载 → E:）。**（2026-09-11 更正）** 本条原写「临时文件 → `c:\Users\PC\.trae-cn\work\...`」，与 AGENTS.md §磁盘与下载规则「**禁止将任何文件写入 C 盘或用户主目录**（`C:\Users\`）」**直接矛盾**；该 C 盘路径虽实测存在（历史遗留产物，非纯笔误），但按 `policy.md` §2 权威链，AGENTS.md 的禁令优先，故更正为项目外 D: 临时目录。**不要把这个 C 盘路径当作可写入的临时目录。**
3. **会话记录确认**：确认 `.trae/execution/sessions/{sessionId}.md` 已在认领时创建（见"任务认领"步骤 3）；若缺失，立即按模板补建后再继续，不得跳过。

4. **验收证据存放自检（2026-09-12 新增）**：**验收证据（关卡/功能验收截图、编译日志等）只放 `Saved/Evidence/{任务号}/`**（例：`Saved/Evidence/M01-T005/`）—— **这是唯一约定**，不得按日期建目录、不得散放他处。
   - **过程性 raw 截图**放项目外临时目录 `D:\AWork\TraeAdmin\VRSanguoYanWuchang\tmp\`，**但交付前必须把属于验收证据的图拷入 `Saved/Evidence/{任务号}/`**。**未拷入 = 验收链取证不完整，属流程欠账，必须在汇报中显式声明**（不得默认「已归档」）。
   - **`Saved/Screenshots/` 是编辑器默认截图目录与过程截图堆，不作为验收证据**；不得在其中指认验收证据。
   - **⚠ 本项无门禁兜底**：`dashboard/check-integrity.py:622-654` 的「验证证据文件路径存在」**只从证据文本中提取 `.trae/` 路径**（L645），**不校验 `Saved/Evidence/...`** ⇒ **证据未归档、证据被删，门禁都不会发现**。**必须靠本条人工纪律，不得依赖工具。**
   - 起因（2026-09-12，用户提出）：用户指出「**好像每个对话放临时截图的位置都不一样**」。实测确有四处口径 —— `Saved/Evidence/{任务号}/`（实际在用）、任务书旧写法 `Saved/Evidence/{日期}/`、`Saved/Screenshots/`（无人管理）、项目外 `TraeAdmin\tmp\`；且 M01-T005 的 v5.8 系列证据**确实未归档**（执行会话自曝）。

铁律验证失败 → 停止，不得修改任何文件。**此规则旨在防止执行模型因上下文缺失而违反磁盘规则或文件边界规则。**

## 执行中沟通检查点（强制）

> **执行模型在以下时机必须主动与用户沟通，不得静默执行全程。**

| 检查点 | 时机 | 沟通内容 |
|--------|------|---------|
| CK-01 | 认领任务后、开始实施前 | "我将开始执行 {taskId}，预计产出 {交付物列表}，是否继续？" |
| CK-02 | 完成 ≥ 50% 步骤后 | 进度简报：已完成步骤、当前状态、是否遇到阻塞 |
| CK-03 | 遇到需要用户决策的事项时 | 明确列出选项和推荐方案，等待用户选择 |
| CK-04 | 遇到阻塞时 | 记录阻塞原因到会话记录，转为 `blocked`，告知用户 |
| CK-05 | 全部步骤完成后、提交审核前 | "全部步骤完成，测试结果 {N/N passed}，是否提交审核？" |

**CK-03 示例场景**：需要用户选择技术方案、需要用户提供外部资源、需要用户在 UE 编辑器中完成操作。

**汇报前置（强制）**：CK-02/CK-04/CK-05 汇报前必须先把本次进展写入会话记录（`.trae/execution/sessions/{sessionId}.md`），先记录、后汇报。

**沟通禁止**：不询问已在 TASK.md/INPUTS.md 中明确的事项；不在每个微小步骤后都询问。

## 会话记录（强制）

> **执行模型必须维护会话记录，确保上下文在会话中断后可恢复。**

### 记录位置

`.trae/execution/sessions/{sessionId}.md`，其中 `sessionId` 与认领时的 `claimedBy` 一致（如 `session-20260825-001`）。

### 记录内容

```markdown
# 会话 {sessionId}

- 开始时间：{ISO 时间}
- 认领任务：{taskId}
- 状态：in_progress / blocked / completed

## 执行进度

| 步骤 | 状态 | 完成时间 | 备注 |
|------|------|---------|------|
| Step 1 | ✅ | 14:30 | xxx |
| Step 2 | 🔄 | — | 进行中 |

## 沟通记录

| 时间 | 检查点 | 内容摘要 |
|------|--------|---------|
| 14:00 | CK-01 | 确认开始执行 |
| 14:30 | CK-02 | Step 1 完成，无阻塞 |

## 关键决策

- {决策内容}（{时间}）

## 阻塞与待办

- {阻塞项}（{时间}）— 等待用户 xxx

## 恢复上下文（供下次会话使用）

- 当前步骤：Step 2
- 已修改文件：{列表}
- 下一步：{描述}
- 需要用户操作：{描述或"无"}
```

### 会话记录的生命周期

- 会话开始时创建，执行过程中持续更新。
- 会话结束时（任务提交审核或阻塞），在记录中标记 `completed` 或 `blocked`。
- 会话记录不删除，供后续会话或审核模型回溯上下文。

## 执行规则

1. 只读取当前任务需要的事实源。
2. 修改前读取最新文件内容。
3. 只修改 `ALLOWLIST.txt` 列出的路径（路径相对于项目根目录）。
4. 使用最小差异，不创建平行系统。
5. 遵守 `systems/` 工程指引中定义的接口契约（核心服务签名、事件广播、数据结构）；实现时不得偏离接口契约的签名和语义，如需调整接口须先提交变更申请。
6. 执行 `CHECKS.md` 和任务指定验证。
7. 如实记录未运行、失败和受环境阻塞的验证。
8. 结果写入任务指定报告路径。
9. 状态最多更新为 `awaiting_review`（同时更新根 STATUS.json 和任务目录 STATUS.json）。

## 后置与阻塞工作登记（强制）

> 实施中产生的任何"后置 / 待办 / 阻塞未完成"事项，不得仅以 STATUS 备注或报告散文存在。

1. **登记锚点**：后置事项必须落入以下之一：(a) TD 登记册（带复审时机）；(b) 任务登记册中的后续任务条目；(c) 根 STATUS.json 对应任务的 `note` 字段（结构化，含恢复条件）。
2. **禁止散文后置**：仅在会话记录、报告正文或代码注释中提及的后置事项不构成有效登记——提交审核自检（CK-05）与审核（ReviewProtocol）均不认可散文后置。
3. **复审触发**：每条后置登记必须写明复审/完成触发条件（如"上游 X 修复后"、"M0N-T00M 开工前"）；上游阻塞解除后，责任会话须触发对应复审（与 ReviewProtocol"死区代码验证债务"联动）。
4. 案例：2026-08-31 BP_VRCharacter 蓝图接线后置仅记于 STATUS 备注散文（且 TD 编号误标 TD-010），数月无人追踪，成为输入链根因温床（CR-20260908-001 B 节 / CR-20260909-001）。

## 共享文件更新

`.trae/CHANGELOG.md`、`.trae/integrity.yaml` 及各登记册为多任务共享文件。更新规则：

- 写入前重新读取最新内容，防止覆盖其它执行模型的并发修改。
- 仅追加或修改与当前任务直接相关的条目，不重写整个文件。
- 如发现冲突（同一行被并发修改），保留双方内容，在 CHANGELOG 中注明。

## 强制停止

- 需要修改白名单外文件。
- 锁定文件哈希不匹配。
- 任务描述有两种显著不同解释。
- 依赖、凭据、插件或设备缺失。
- 编译或测试失败，且任务规定的重试次数已经用尽。
- 发现现有架构与任务假设冲突。
- 与另一 `in_progress` 任务产生文件冲突。
- **CHECKS.md 检查项因 ALLOWLIST 缺口无法完成**：当 `CHECKS.md` 或 `TASK.md` 中要求的交付物/验证项需要修改 ALLOWLIST 未列出的路径时，不得跳过该检查项或将任务标记为 `awaiting_review`。必须停止，报告 ALLOWLIST 缺口，等待决策模型或用户扩充白名单后再继续。

停止后状态设为 `blocked`（更新根 STATUS.json），记录原因、证据和恢复条件。

## 禁止

- 不批准自己的结果。
- 不修改总纲、Skill、治理政策和执行入口。
- 不执行下一任务。
- 不把未验证内容写成通过。
- Git 未启用前不执行 Git；启用后也不得执行破坏性或提交类命令，除非任务明确授权。