# rebuild_v5.py — L_Prototype_1v1_v5 校场单源生成脚本（M01-T005；v5.2 旗侧挂/鼓落台面/Docs；v5.3 坛面台阶修复；v5.4 旗统一东风+扁旗面；v5.5 坛厅分离/靶场重排/文档 HTML+实测俯视图；v5.6 圆草靶/北辕门撤除/平面图实测 bounds；v5.6b 鼓亭落地/两翼敞棚/md 恢复；v5.7 靶薄盘+红心贴面/东踏步撤除/廊庑功能化；v5.8 坛中轴后移/单体选将殿东邻/坛东梯；v5.8b 东梯回退主梯规格；v5.8c 坛扩 28x14/顶层清空/南梯加宽/二层立鼓；v5.8d 立鼓改堂鼓式立盘鼓）
...
T1_W, T1_D, T1_H = 2800, 1400, 170   # 一层 28x14x1.7（v5.8c 面积扩大：二层容立鼓+站人；南立面 4300；北立面 5700 距北墙 2.45m）
T2_W, T2_D, T2_H = 2200, 1100, 340   # 二层 22x11x3.4（北立面与一层齐）
T3_W, T3_D, T3_H = 1400, 700, 500    # 三层 14x7x5.0
T1_Y = 5000                          # v5.8 用户拍板：坛后移 +900（原 4100），中轴留白，厅东邻
T2_Y = T1_Y + (T1_D - T2_D) / 2.0        # 二层北立面与一层齐
T3_Y = T1_Y + (T1_D - T3_D) / 2.0 - 100  # 三层北面较二层北面内收 100（原版式）
ESTEP_N = 5                          # 东梯每段 5 步（3 段共 15 步到顶）
ESTEP_W = 240                        # 东梯宽 2.4m（v5.8b 用户"别缩短"：步距/坡度与南主梯同规格）
...
# ---------- 4. 主坛：三层退台（平乐观/拜将坛坛制；v5.8 中轴后移+900 作北端背景板；v5.8c 面积扩大 24x12.5→28x14）----------
box('SM_Jiangtai_Tier1', (0, T1_Y, T1_H / 2.0), 0, (T1_W / 100.0, T1_D / 100.0, T1_H / 100.0))
box('SM_Jiangtai_Tier2', (0, T2_Y, (T1_H + T2_H) / 2.0), 0, (T2_W / 100.0, T2_D / 100.0, (T2_H - T1_H) / 100.0))
box('SM_Jiangtai_Tier3', (0, T3_Y, (T2_H + T3_H) / 2.0), 0, (T3_W / 100.0, T3_D / 100.0, (T3_H - T2_H) / 100.0))

# 三段南主梯（每段 5 步，步高 33.3cm < NavMesh 35cm 限制；每段完全落在承载面上；v5.8c 加宽 500/300→800/600 气派）
...
stair_segment('A', 0.0, T1_Y - T1_D / 2.0, 800)          # 地面 -> 一层顶 1.7m（贴一层南立面 4300）
stair_segment('B', T1_H, T1_Y - T1_D / 2.0 + 300, 600)   # 一层顶 -> 二层顶 3.4m
stair_segment('C', T2_H, T1_Y - T1_D / 2.0 + 600, 600)   # 二层顶 -> 三层顶 5.0m

# 坛东梯（v5.8 用户设计：面向演武厅的登坛线，选将后由此上坛受令。v5.8b 用户"东侧楼梯太奇怪了，别缩短"：
# 回退与南主梯同规格——15 步 x 33.3cm 步高 x 50cm 跑深，坡度一致；差别只剩位置（东面）与宽度 240）。
# 三段贴三层东立面逐层上：段1 地面->T1 顶；段2 T1 顶条带->T2 顶；段3 T2 顶条带->T3 顶。
def east_stair_segment(tag, base_z, face_x):
    for k in range(1, ESTEP_N + 1):
        h = T3_H * k / 15.0
        x = face_x + (ESTEP_N - k) * STEP_RUN + STEP_RUN / 2.0
        box('SM_Jiangtai_StepE_%s_%02d' % (tag, k), (x, 5100, base_z + h / 2.0), 0,
            (STEP_RUN / 100.0, ESTEP_W / 100.0, h / 100.0))
east_stair_segment('A', 0.0, T1_W / 2.0)    # 地面 -> 一层顶
east_stair_segment('B', T1_H, T2_W / 2.0)   # 一层顶条带 -> 二层顶
east_stair_segment('C', T2_H, T3_W / 2.0)   # 二层顶条带 -> 三层顶

# v5.8c（用户）：顶层护栏整体删除（"围墙好奇怪，直接删掉吧"）——坛顶为开放台面；
# 顶层点将桌（用户问"立方体干啥的"=点将桌，点将职能已移选将殿）与三面战鼓一并撤除。
...
# 二层东南/西南角立鼓x2（v5.8d 用户："立起来的鼓是一个立起来的圆饼"——堂鼓式立盘鼓：
# 底座+南北两立柱+承梁，鼓=立式圆盘 dia140 t55（SM_Sphere 透镜盘竖放，双面圆盘即双面鼓皮，面朝南北），
# 皮心高台面上 1.25m 可敲；M02 接打击交互）
for sgn, tag in ((1, 'E'), (-1, 'W')):
    dx, dy = sgn * 800, T1_Y - 150
    box('SM_DrumStand_%s_Base' % tag, (dx, dy, T2_H + 15), 0, (1.4, 2.0, 0.3))
    pole('SM_DrumStand_%s_Post_N' % tag, (dx, dy - 90, T2_H + 30), 130, 24)
    pole('SM_DrumStand_%s_Post_S' % tag, (dx, dy + 90, T2_H + 30), 130, 24)
    box('SM_DrumStand_%s_Beam' % tag, (dx, dy, T2_H + 40), 0, (0.5, 1.8, 0.4))
    disc('SM_DrumStand_%s_Drum' % tag, (dx, dy, T2_H + 125), 140, 55, yaw=0)   # 立盘双面鼓皮（yaw=0 面朝南北）

# 三层顶：帅旗居中（点将桌/三鼓已撤——v5.8c）
pole('SM_Jiangtai_ShuaiFlag_Pole', (0, T1_Y + 450, T3_H), SHUAI_H, 40)
box('SM_Jiangtai_ShuaiFlag_Flag', (160, T1_Y + 450, T3_H + SHUAI_H - 150), 0, (3, 0.06, 1.4))  # 东风统一

# 碑石×2（拜将坛制：台脚东西立碑；v5.8c 随坛扩外移，避让东梯 y4980..5220 与鼓亭 y4490..4710）
for sgn, tag in ((1, 'E'), (-1, 'W')):
    box('SM_Stele_%s' % tag, (sgn * 1600, T1_Y - 100, 130), 0, (1, 0.4, 2.6))
text('Text_JIANGTAI', (0, T1_Y - 1150, 750), 'JIANGTAI 28x14x5m 3-TIER', 80, yaw=270)

# 坛后北缝遮挡（用户"留几米或用物体遮挡"）：军旗x5+器械架x2 封坛后 3.2m 袋状死区（y5625..5945）
for i, fx in enumerate((-800, -400, 0, 400, 800)):
    banner('SM_NorthGap_Flag_%d' % i, fx, 5760, 400, 120, 80, dia=20, face='+X')
for sgn, tag2 in ((1, 'E'), (-1, 'W')):
    box('SM_NorthGap_Rack_%s_Base' % tag2, (sgn * 1000, 5800, 10), 0, (0.6, 2, 0.2))
    for j in range(3):
        box('SM_NorthGap_Rack_%s_Blade_%d' % (tag2, j), (sgn * 1000, 5725 + j * 75, 80), 0, (0.08, 0.4, 1.4))
    box('SM_NorthGap_Rack_%s_Bar' % tag2, (sgn * 1000, 5800, 160), 0, (0.6, 2, 0.1))

# ---------- 5. 演武厅（v5.8 单体敞殿 16x10 @ (3000,5100)，选将殿；用户拍板布局：坛中轴后移作背景板，
# 厅东邻坛、西门朝坛，面向西望场心与南半场无遮挡）----------
# 选将玩法位（M02 接机制）：厅内北墙=将位陈列 x8 基座；选将动线 出西门->坛东短梯登坛受令->南主梯下场。
# 形态可切换（汉风木构殿/匈奴穹庐帐/明式歇山）：灰盒只锁位置与占地，形态=后续参数组。
HX, HY = 3000, 5100
HW, HD, HH = 1600, 1000, 565
box('SM_Hall_Back', (HX, HY + HD / 2.0 - 25, HH / 2.0), 0, (HW / 100.0, 0.5, HH / 100.0))   # 北墙=陈列墙
box('SM_Hall_East', (HX + HW / 2.0 - 25, HY, HH / 2.0), 0, (0.5, HD / 100.0, HH / 100.0))   # 东墙
box('SM_Hall_South', (HX, HY - HD / 2.0 + 25, 60), 0, (HW / 100.0, 0.5, 1.2))               # 南矮墙 1.2m
for i in range(5):                                                                          # 西檐柱x5（西门敞口）
    pole('SM_Hall_Post_%d' % i, (HX - HW / 2.0 + 25, HY - 400 + i * 200, 0), HH, 40)
box('SM_Hall_Lintel', (HX - HW / 2.0 + 25, HY, HH - 25), 0, (0.4, 9.2, 0.5))                # 西檐楣（顶 565 贴顶板）
box('SM_Hall_Roof', (HX, HY, HH + 15), 0, (HW / 100.0 + 0.5, HD / 100.0 + 0.5, 0.3))        # 顶板 565..595
for i in range(8):                                                                          # 将位基座x8（厅内北墙）
    pole('SM_Hall_Warrior_%d' % i, (HX - 700 + i * 200, HY + HD / 2.0 - 120, 0), 110, 60)
text('Text_HALL_SELECT', (HX, HY + HD / 2.0 - 80, 400), 'WARRIOR SELECT HALL', 55, yaw=270)
text('Text_HALL_FACE', (HX - HW / 2.0 - 60, HY, 320), 'REVIEW HALL 16x10m', 55, yaw=90)
...
- **v5.8d（本次，用户"立鼓是立起来的圆饼，你这还是坐在地上的鼓"）**：二层立鼓重制——v5.8c 的立式
  圆桶+顶盖读作"坐地桶"，改为**堂鼓式立盘鼓**：底座+双立柱+承梁，鼓身=透镜圆盘 dia140 t55 竖放
  （双面均是鼓皮，面朝南北），皮心高台面 1.25m。props 384→388。其余不变。
- **v5.8c（用户 4 条）**：①**顶层清理**：护栏整体删除（"围墙好奇怪，直接删掉"）；点将桌删除
  （用户问"立方体干啥的"=点将桌，点将职能已移选将殿）；顶层三面战鼓删除；帅旗居中重摆。
  ②**南主梯加宽**（"不够气派"）：A 段 500→800、B/C 段 300→600。③**坛体扩大**（"面积该再扩大，
  否则二层放鼓没位置站人"）：三层 24x12.5/18x10/12x6 → **28x14 / 22x11 / 14x7**（高度不变 170/340/500，
  北立面 5700 距北墙 2.45m）；碑石外移 (±1600,4900)、鼓亭外移 (±1650,4600) 避让新坛体。
  ④**二层东南/西南角立鼓x2**（要"立起来可以敲的鼓"）：堂鼓坐架立放（基座+立式鼓身 dia110 + 顶面鼓皮
  dia120，皮面高约 1.48m 可敲），M02 接打击交互；点位 (±800,4850) 距 C 梯 5m、距坛缘 3m 留站人空间。
  props 388→384（-护栏6 -桌1 -三鼓3 +立鼓6）；save True+R；Docs 五件同代；Nav 复测全绿+新增二层→立鼓链。
- **v5.8b（用户"东侧楼梯太奇怪了，别缩短"）**：坛东梯回退与南主梯**同规格**——15 步 x 33.3cm 步高
  x 50cm 跑深（v5.8 首版为省进深用 25cm 跑深，坡度过陡读作奇怪构件），坡度与南主梯一致，差别只剩
  位置（东面朝厅）与宽度 240；高度基准/护栏开口/寻路链路不变，Docs 五件同代重生成。
- **v5.8（用户"按图动工"+要求文档同步）**：**北侧布局重排——用户方案实施**：
  ①**拜将坛中轴后移 9m**（中心 y4100→5000，坛后缘 5625 距北墙 3.2m）作北端背景板，1v1 中轴保持对称，
  南向登台进深放开到 43m；②**演武厅重建为单体敞殿 16x10 @ (3000,5100)**（v5.6b 两翼敞棚拆除——用户
  "敞篷确实不合适"），东邻坛、西门朝坛，面向西望场心与南半场无遮挡；厅内北墙=将位陈列 x8 基座+标注
  WARRIOR SELECT（M02 选将机制位）；③**坛东短梯（带职能回归）**：用户设计"面向演武厅这侧设短一点
  的楼梯"——三段贴东立面 15 步（34/34/32cm，低于 Nav35 限制；v5.5b 教训：42cm 档 NavMesh 拒连风险），
  跑深 375/段、宽 2.4m，短于南主梯；选将动线=出厅西门→东短梯登坛→坛顶受令→南主梯下场；④**坛后
  北缝遮挡**：军旗x5+器械架x2 封 y5625..5945 袋状死区（用户"留几米或用物体遮挡"取后者）；⑤鼓亭x2/
  碑石x2 随坛北移并避让东梯；⑥东侧护栏留东梯口（南 0.5m+北 3m 段）。验证：props 对账 353→387；
  Nav 新链路（厅内选将位/厅→东梯→坛顶/坛顶→南主梯→场心/厅前到达）+存量链路复测；Docs 五件同代重生。
- **v5.7**：①**靶面朝向 90° 根修**（disc 扁轴在 Y=盘面侧对射手，v5.6 注释误写"朝东西"；用户看到的"蛋"
  是 8cm 盘缘的透镜轮廓，"浮球"是红心缘的视差脱开）：disc 增 yaw 参数，靶盘/红心 yaw=90 面朝东对射手，
  盘厚 30→8cm、红心贴面凸 2cm——正对射手见完整圆盘+盘心红心；**判读教训：证据图核验必须先验朝向假设**；
  ②**将台东侧服务踏步撤除**（用户"侧边突然多了个楼梯，怎么引入"）：v5.5 无职能服务线读作多余侧梯，
  登坛只走南主台阶，东侧护栏恢复整段（v5.8 因用户设计"面向厅的东短梯"带职能回归）；③**廊庑功能化**
...
MODULES = [
    ...
    dict(id='altar', name='三层拜将坛', cat='仪式', anchor=(0, 5000), size=(2800, 1400, 500),
         source='平乐观大坛(188)+拜将坛遗址', func='点将/授印/颁奖高点+北端背景板（v5.8 中轴后移 9m，北缝军旗器械架遮挡；v5.8c 面积扩至 28x14、顶层护栏/点将桌/三鼓撤除、南主梯加宽 800/600、二层东南西南角立鼓x2 可敲）', assets='材质：夯土+石包边', prio='M02 白模'),
    dict(id='east_stairs', name='坛东梯', cat='结构', anchor=(1400, 5100), size=(250, 240, 500),
         source='v5.8 用户设计：面向演武厅的登坛线；v5.8b 用户"别缩短"回退与南主梯同规格（15 步 33.3cm、跑深 250/段）', func='选将后登坛受令动线；三段贴东立面，步高 33.3cm 同南主梯', assets='材质：石阶', prio='M02 白模'),
    dict(id='drum_pavilions', name='鼓亭×2', cat='仪式', anchor=(1650, 4600), size=(220, 220, 300),
         source='平乐观"亲执枹鼓"', func='指挥鼓点（坛脚东西各一，不占坛面服务路线）', assets='模型：木构鼓亭+战鼓', prio='M02 白模'),
    dict(id='command_deck', name='坛面鼓旗组', cat='仪式', anchor=(800, 4850), size=(2200, 1100, 300),
         source='v5.8c：顶层三鼓/点将桌撤除；v5.8d 用户"立鼓是立着的圆饼"改堂鼓式立盘鼓x2（M02 打击交互）', func='立盘鼓x2 二层东南/西南角（立式双面盘鼓，皮心高 1.25m 可敲）+帅旗 10m 居中', assets='模型：立盘鼓+鼓架+帅旗', prio='M02 白模'),
    dict(id='steles', name='碑石×2', cat='仪式', anchor=(1600, 4900), size=(100, 40, 260),
         source='拜将坛台脚立碑', func='场景叙事锚点', assets='模型：石碑+碑文贴图', prio='M03'),
    dict(id='hall', name='演武厅（选将殿）', cat='建筑', anchor=(3000, 5100), size=(1600, 1000, 595),
         source='唐"阅武堂"/宋"演武亭"；v5.8 用户拍板：单体敞殿 16x10 东邻坛、西门朝坛（厅面向西望场心与南半场无遮挡）',
         func='选将场景（M02 接机制：厅内北墙将位陈列 x8）+点将/受令；形态可切换：汉风木构殿/匈奴穹庐帐/明式歇山（灰盒只锁位置占地）', assets='模型：木构殿+将位基座x8', prio='M02 白模'),
