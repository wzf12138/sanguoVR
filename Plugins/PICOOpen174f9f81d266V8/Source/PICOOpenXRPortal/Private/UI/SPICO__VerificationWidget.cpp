 ...
 */
TSharedPtr<SVerticalBox> SPICOXRProjectVerificationWidget::BuildVerificationRow(TObjectPtr<UPICOVerificationObject> InObjectPtr)
{
	if (InObjectPtr)
	{
	...
	else
	{
		...
		case EVerificationLevelPICO::Recommended:
			...
			break;
		case EVerificationLevelPICO::None:
			{
				IconBrush = FAppStyle::Get().GetBrush("Icons.ErrorWithColor");
			}
			break;
...
FReply SPICOXRProjectVerificationWidget::OnFixClicked(TObjectPtr<UPICOVerificationObject> InObjectPtr)
{
	if (ObjectPtr)
	{
...
{
	return UPICOPortalFunctionLibrary::HasVerificationNeedRestart() ? EVisibility::Visible : EVisibility::Collapsed;
}

FReply SPICOXRRestartEditorWidget::HandleRestartEditorButtonClicked() const
{
	const bool bWarn = false;
	UPICOPortalFunctionLibrary::SetVerificationNeedRestart(false);
